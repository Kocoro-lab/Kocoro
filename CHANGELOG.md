...
- **Compaction trigger and summary reliability** — estimator calibration now survives daemon turns with model/tool-registry validation; proactive, preflight, and reactive gates share calibrated thresholds and land below their trigger with explicit hysteresis. Summaries are structure-audited, retried once when malformed, and mechanically preserve identifier-shaped values. Oversized summarizer input folds tool-pair-safe chunks instead of silently losing the middle; proactive/preflight compaction restores recent file reads within the landing budget, while reactive recovery deliberately skips restoration after a provider context-length rejection.
