...
> **Cross-repo contract:** five new **additive** capability tokens — `computer_use_topology_v1`, `computer_use_control_v1`, `computer_use_app_policy_v1`, `computer_use_physical_interference_v1`, `computer_use_risk_confirmation_v1` — on the WS handshake (`X-Kocoro-Capabilities`) and `GET /status`. Desktop must gate each surface on its token; when a token is present, transport/decode/endpoint errors are degraded fail-closed states, never a signal to fall back to legacy UI. **The provider-native path additionally depends on a Cloud `POST /v1/completions/resolve` endpoint that is not deployed yet.** Until it is, every run resolves 404 and safely downgrades to the AX-only generic `computer_use` — the feature is dark, not broken. Note the downgrade is deliberately narrow: only 404/405 and transport errors fall back, while 401/403/409/5xx and any malformed profile fail the run closed, so a half-deployed Cloud surfaces loudly instead of silently selecting the wrong adapter. Verify one real resolve end-to-end before relying on provider-native execution.
