...
## Unreleased — agent @mentions with conversation roster forwarding

Lets the agent decide to @mention other users by name in IM replies (Teams first, Slack/Feishu later). Cloud forwards the live conversation roster on inbound messages; the daemon renders it into sticky context as a bulleted `Conversation participants:` list, and the system prompt teaches the agent to write `@<display name>` inline using the exact roster name. Cloud resolves the name to the platform's user identifier (Teams `aadObjectId` / `29:…`, Slack `U…`) at send time; unresolved names degrade silently to plain text. The agent never sees nor writes internal IDs.

> **Cross-repo contract:** all additive, no breaking wire changes. `MessagePayload.Participants []string` (Cloud → daemon) and `ReplyPayload.Mentions []Mention{Name, Email}` (daemon → Cloud) are mirrors of shannon-cloud's same fields, both `omitempty` so either side can ship first (legacy peer omits the field; the consumer falls through to a safe fallback). The roster path is observability-gated by the new `mention_roster_v1` capability token (consumer: Cloud — gates whether to populate inbound `participants` and to parse outbound `@name` from reply text); ReplyPayload.Mentions is reserved scaffolding (no daemon producer today — duplicate-display-name disambiguation is Cloud-side-only). The new `Conversation participants:` sticky-context block does not change any existing line; routing decisions never read it.
