...
- **Conversation context actions** (`conversation_context_actions_v1`, #347) — a Desktop-only local surface for working sideways off a conversation without disturbing it. `POST /sessions/{id}/fork` copies the transcript through a **complete assistant turn** into an ordinary persisted session (optionally under a different `target_agent`, validated to exist before the fork lands so a typo cannot file it where no UI lists it); `POST /sessions/{id}/side-chat` runs one ephemeral turn over that same bounded history. `message_index` on both is an index into the RAW archive — system-injected entries counted — the same index space Desktop already uses, and a cut in the middle of an assistant/tool trajectory is rejected (`ErrIncompleteTurnBoundary`) so copied context can never contain a `tool_use` without its result. A compaction checkpoint covering the requested prefix is honored rather than re-expanded: the fork deep-copies it (dropping one whose coverage runs past the cut), and the side chat feeds checkpoint-plus-tail through the new `Session.HistoryThrough`. A fork inherits conversation context, CWD, project, and message timestamps — never runtime state (no work plan, usage ledger, route binding, share record, or in-progress execution).
- **Side chats keep full capability but no footprint** — the ephemeral run uses the normal tool registry and permission engine, with approvals over the per-request SSE stream exactly like `/message`. Nothing persists: no session, no bus events, and deliberately **no question asker**, because the panel has no question UI and an asker there would block the run for the whole resolution window before reporting a decline the user never made — the failure mode that once hit the IM channels. `ask_user_question` degrades to its clean "can't ask here" result instead.
- **Desktop reply annotations** — text replies ride a transient head-only `<kocoro_replies>` envelope that reaches the model but is stripped from the archive **only when it parses**; a delimited-but-malformed envelope is kept verbatim, so user bytes are never silently dropped. Decoded replies persist as `message_meta[].conversation_annotations` so a client can rebuild the sent-message attachment on reload. Limits (100 replies / 8K-rune quote / 2K-rune comment) are enforced against the exact envelope the model would receive, with stable `conversation_replies_*` / `conversation_reply_*` error codes at both `/message` and `/queue`. Consecutive head envelopes — produced when a drained mailbox batch merges with the triggering message — are consumed as one run, and each drained message is split individually so a merged turn keeps per-message annotations.
- **`memory_recall` aggregation and temporal status** — `direct_relation` accepts an `aggregator` of `count` or `sum` for how-many / how-much questions on a named fact, rejected at validation on the other two modes; the tool description steers the model toward an explicit `YYYY-MM-DD/YYYY-MM-DD` window rather than a relative phrase. Recall groups and candidates now carry `temporal_status` (`current` vs `superseded_by_recency`) end to end — through the wire types, the tool result, and the injected `<private_memory>` block — so a fact the user has since replaced is not answered as if it still held.
- **Unknown sidecar fields survive the Go round-trip** — `QueryCandidate` and `MemoryCandidateGroup` keep any JSON key the daemon does not model (`aggregation`, `measure`, …) in an `Extra` map and re-emit it, so a field the sidecar adds later reaches the model instead of being dropped at the type boundary.

### Fixed

- **A rejected aggregator no longer produces a fake total** — when the sidecar rejects `aggregator` as a schema violation, the tool retries once without it and tags the result `structured_unaggregated` plus an explicit `aggregator_unsupported` warning telling the model **not** to read the returned list length as a count — that list is truncated at `result_limit`, so its length is an artifact of the cap, not a total. The fallback is narrow by construction: it fires only on a schema/validation error that actually names the `aggregator` field, so an unrelated failure is not silently downgraded into a different query. A withheld count arrives as `reason=incomplete` (surfaced as `structured_incomplete`) — a known lower bound, never a finished total.
...
- **Durable per-run work plans** (`work_plan_v1`) — `set_work_plan` records a full-snapshot progress checklist (2–8 steps, cap configurable via `agent.work_plan_max_steps`) for one execution run. The model owns only step content and status plus an optional explanation; `plan_id` / `run_id` / `revision` / `lifecycle` / `close_reason` / `updated_at` are runtime-owned and are never accepted as tool arguments. Ownership binds to the stable `RunID`, so an interrupted-run recovery attempt continues the same plan rather than starting a new one. Registered on every non-ephemeral daemon run — Fast and Full profiles alike, never gated on query shape or whether anything is watching — so the provider-visible schema stays byte-stable across equivalent runs; ephemeral runs skip it because a durable record could not honor its own contract without session persistence.
- **Persist-before-emit ordering** — a changed snapshot sets `ToolResult.CheckpointNow`, which bypasses the checkpoint debounce and makes the durable save a precondition of continuing (a save failure fails the run rather than leaving a dirty bit). Only after that save succeeds does `work_plan.updated` leave the pending slot, so SSE can never display a revision that a daemon crash would erase. Identical normalized snapshots are a no-op: no revision, no event, no forced write.
- **Runtime-owned closure** — at run end the plan closes from `LastRunStatus`: cancellation, then partial, then hard error, then completed-vs-pending. A clean run that left steps unchecked closes `stopped` / `run_completed_with_pending_steps` **without downgrading the outer run result** — stale steps are model metadata, not failure evidence. Closure bumps the revision, because consumers drop lower-or-equal revisions and a terminal transition riding the previous number would be discarded as stale. Recovery paths that finalize a session without entering the agent loop close orphaned plans explicitly, and history mutations (`Reset`, `TruncateMessages`, `TruncateAt`) drop the plan outright — the transcript that justified it is gone.
- **Wire surface** — the dotted bus event `work_plan.updated` (fixture `bus_event.work_plan.updated.json`) plus an optional `work_plan` field on `GET /sessions/{id}`, which is the recovery authority; the `?view=remote_timeline` projection deliberately does not carry it. Full snapshot + monotonic revision is the recovery unit: clients drop lower-or-equal revisions of the same `plan_id`, may coalesce under backpressure, and refetch the session detail after a gap. Additive and capability-gated, so an older client that never decodes it keeps working. Plan state must never drive the outer task lifecycle.
- **`agent.work_plan_max_steps`** (default 8) — bounds checklist length so long multi-stage runs summarize into coarse stages and the progress card stays glanceable. When it binds, the tool returns a `[validation error]` telling the model to merge stages.

### Changed

- **Prompt guidance is a byte-exact strip section** — the `## Work Plans` block is removed at prompt-build time wherever `set_work_plan` is not registered, so TUI, one-shot CLI, and MCP prompts are unchanged and prompt-cache prefixes do not fragment. Koe Realtime never sees the tool at all (its `ToolDefs` are a hardcoded literal with no registry connection), pinned by a tripwire test. A resumed run re-anchors its own active plan through VolatileContext after `cache_break`, so recovery survives a compaction that dropped the original call.
- **Step content is whitespace-collapsed before it is stored** — models can emit newlines inside a JSON string, and the plan is rendered one line per step. Storing raw content would let a single embedded newline forge checklist lines the runtime never recorded, and because the duplicate/no-op comparison already collapsed whitespace, a model resubmitting the corrected text would be told nothing changed and could never repair its own step. Collapsing at ingestion keeps "identical normalized snapshots are no-ops" true of what is actually stored; the render path collapses again so a plan written by an older binary cannot reintroduce the problem. The 200-rune step cap is consequently measured after collapse.

## v0.4.4 — 2026-08-16 — Outcome-aware execution + crash-safe external writes

**A run now commits to a single, honest outcome, and every material external write is journaled so an interrupted run knows what already took effect.** The agent loop previously had no durable notion of "this tool call reached the outside world" — a crash between dispatching a write and reading its response left the next run free to replay it, and a run that ran out of provider budget mid-task could still report as if it had finished. This release adds a content-free side-effect journal around each mutating `Tool.Run`, teaches the loop to answer with exactly one of *completed / partial / blocked / outcome-unknown*, and surfaces partial results explicitly on the wire instead of dressing them up as completion. Alongside it: two offline tools that keep arithmetic and clock questions off `web_search`, provider-neutral response-style control, and an MCP connect that actually retries.

### Added

- **Crash-safe side-effect journal** (#338) — `internal/session/tool_execution.go` plus `side_effect_journal.go` in both `internal/agent` and `internal/daemon` persist a dispatch state machine around every material external write: `prepared → dispatching → committed / failed_no_effect / outcome_unknown / abandoned`. The pre-dispatch checkpoint is durable before the tool runs, so a crash between dispatch and response resolves to `outcome_unknown` rather than a silent replay — `ErrSideEffectOutcomeUnknown` tells callers **not** to auto-retry. Records are content-free (tool-use-id / arguments / result stored only as digests); a capable tool with a real native idempotency contract may consume the exposed `IdempotencyKey`, but a generic provider does not become idempotent merely because the key exists. Terminal history is bounded at `MaxRetainedTerminalToolExecutions` (256) — the oldest completed audit rows are trimmed while every unresolved / outcome-unknown record is kept.
- **Explicit partial-run delivery** (`schedule_run_partial_v1`, #338) — a run stopped by the provider budget (or any soft stop) now returns a usable best-effort reply carrying `partial: true` and a stable `failure_code`, instead of being reshaped into a generic completion. The distinction is delivered end to end through schedules and the Desktop `done` event; the loop is prompted to declare an **Outcome** (completed / partial / blocked / outcome-unknown) and reserve *completed* for when tool results explicitly prove every required result. New wire fixture `sse_event.done.partial.json` pins the shape — UI must not render a `partial: true` result as verified completion.
- **`calculate` and `current_time` local tools** (#338) — two Direct, read-only, no-network tools. `calculate` evaluates arithmetic locally (decimal/integer literals, parentheses, unary `+`/`-`, and `+ - * / %`); `current_time` reads the system clock with an optional IANA timezone (`Asia/Tokyo`, `America/New_York`, …). Both descriptions steer the model off `web_search` for work they can answer exactly.
- **Provider-neutral response-style control** (`agent_response_detail_v1`, #339) — `agent.response_detail` selects the visible-answer detail profile: `concise`, `balanced` (default), or `detailed`, validated in `validateConfig` and `PATCH /config`. It renders into the system prompt as a `<response_detail>` block only (cache-safe — no schema or message divergence), is honored across providers, survives `SwitchAgent` as a per-agent override, and is suppressed on internal structured-output lanes. The capability is advertised on the WS handshake and `GET /status` so clients can expose the control.
- **Run-event and warm-set caps are now operator-tunable** (#338) — `agent.run_event_retention` (default 32) bounds observation-only attempt logs per session; `agent.run_event_max_age_days` (default 14) ages out run-event JSONL, incomplete markers, and lock files; `agent.warm_set_max_schemas` (16) and `agent.warm_set_max_schema_tokens` (8000) bound the deferred-schema warm set. All are validated (negative values rejected; `0` = disabled where documented).

### Fixed

- **A failed MCP connect stayed dead until daemon restart** (#340) — a failed async connect was terminal: the result callback logged and audited it and returned, so a server that lost its single startup attempt stayed "enabled but disconnected", and the one recovery helper was reachable only from the reload branch a credential change never takes. Rotating a server's token (which rebuilds the manager and respawns the stdio child) could race the SIGTERM'd process group and fail that lone attempt for good. New `mcp.ReconnectScheduler` retries per server with exponential backoff (5s doubling to a 5-min cap, 6 attempts spanning ~5m15s), one pending retry per server, streak reset on success, and `Forget` as the re-arm hook for explicit user action. Both connect paths — daemon startup and the reload rebuild — share one `buildMCPConnectResult` callback, retries are owned by the manager generation that spawned them, and `SwapMCPReconnectScheduler` / `ShutdownCleanup` stop a superseded ladder before it can respawn a subprocess the cleanup is reaping.
- **`http` blindly retried mutating requests of unknown outcome** (#338) — a `POST`/`PUT`/`PATCH`/`DELETE` that was dispatched but produced no response now returns an explicit *outcome UNKNOWN — verify before retrying* result rather than letting the model resend it; a `RoundTripper` hook tracks whether request bytes / headers were written so "may have started" is decided from the wire, not a guess. Safe methods (`GET`/`HEAD`/`OPTIONS`/`TRACE`) are unaffected.
- **Integration connect failures were undiagnosable offline** — when the connect passthrough to Cloud fails, the daemon now records a structured WARNING with provider, HTTP status, the contract error code/message (or the raw body when it doesn't parse), and elapsed time; transport failures are logged distinctly. Response-side content only — the request body may carry long-lived credentials and is never logged — and every logged value is capped at 2 KiB.

### Changed

- **`grep` gains output modes and per-file caps** (#338) — the tool now returns matching **file paths only** by default (`output_mode=files_with_matches`) to keep results small; `output_mode=content` returns `file:line:text` and `output_mode=count` returns per-file match counts. Matches are capped per file with a search receipt reporting observed vs. returned counts and whether the cap bound, so a broad pattern degrades to a measurable, narrow-able result instead of a flood.
- **Fast Koe routing and exact-model outcome routing** (#338, `ec98c80f`) — Fast-lane Koe execution and exact-model requests now go through the provider outcome contract, preserving the selected effort/reasoning intent; `modelcontext.go` / `modelmaxtokens.go` gain the exact-model entries the contract needs.
- **Docs and live-eval overhaul** (#342, #343) — the agent-facing guides (`AGENTS.md`, `CLAUDE.md`, kocoro skill `references/*`) were corrected against the code on disk and given a link-check backstop (`scripts/check-doc-links.sh`) so they stop drifting; `docs/cloud-contract-surface.md` documents the Cloud contract surface. The test suite was simplified and gained an outcome-driven live-evaluation harness (`scripts/agent-lab.sh`, `generate-agent-harness-report.py`, and the `test/e2e/*_live_test.go` / `*_contract_test.go` additions) plus `koe-provider-qualification.sh`.

## v0.4.3 — 2026-08-08 — Compaction parity + answer delivery

**v0.4.2 made compaction durable in the daemon; this release brings the TUI's manual `/compact` onto that same pipeline and makes every compaction pass visible.** `/compact` had drifted on four axes — a hardcoded 128K window, zero estimator calibration, a 60s deadline that aborted exactly the sessions large enough to need folding, and no file restoration — so on a 1M-window session it over-compacted by 8x and then failed the ones it mattered for. It also had no exclusion against agent runs in either direction, letting a compact worker and a run goroutine write the same session concurrently.

Separately, the force-stop synthesis turn could deliver the user nothing: it read only `OutputText`, so an HTTP-success response whose visible text lived in `ContentBlocks`, or one whose reasoning budget swallowed the answer entirely, silently became the generic fallback line with nothing recorded.

### Added

- **Compaction status events** (`compaction_status_events_v1`) — only compaction *failures* emitted a run-status event, so a multi-second summary call was indistinguishable from a hang. Every pass (proactive, preflight, reactive, force-stop) is now bracketed with `compaction_started` / `compaction_finished` on the existing `run_status` channel, `detail` = phase. `finished` always pairs with `started` on every exit — applied, shaping no-op, or summary failure — so an indicator keyed on `started` cannot stick. The codes are additive on an existing event shape and consumers treat unknown codes as informational, so an older client simply never shows the indicator; the token exists so a client can distinguish "older daemon without the protocol" from "this run never compacted". The TUI labels its spinner "Tidying context" between the two, with the run-end message as a backstop.
- **`agent.compact_timeout_secs`** (default 300) — the whole manual `/compact` pass shared a 60s deadline, which aborted exactly the oversized sessions whose summarize folds into up to nine sequential small-tier calls. Now operator-configurable per the Hardcoded Limit Policy; negative values are rejected in `validateConfig` alongside the neighbouring timeout fields.
- **Post-compaction file restoration in the TUI** — `/compact` reuses the loop's restoration builder and the TUI's long-lived `ReadTracker`, so exact file content the summary only paraphrases comes back. The task reanchor is deliberately skipped — the user's next prompt is the anchor. The block is skipped when the shaped tail already ends with a user message, because `SanitizeCompactedHistory`'s keep-later merge would delete that earlier message on the next load and orphan-strip its paired `tool_use`.
- **Token-mode integration connect** (`integration_connect_body_v1`) — providers that cannot use the broker's browser OAuth flow (Shopify) now send credentials in the connect body and get back an active connection directly. `POST /integrations/{provider}/connect` forwards the client's JSON verbatim (64 KiB cap, 413 when it binds) and sets `Content-Type` only when a body is present, keeping OAuth providers byte-identical on the wire. A whitespace-only payload counts as absent so a renderer quirk cannot flip an OAuth connect onto the token-mode shape. The body may carry long-lived credentials and is never logged or persisted; the verbatim response passthrough relies on Cloud keeping connect error bodies credential-free. The async integration-tool refresh is pinned 2xx-only, so a rejected credential passes through without registering tools. Desktop must gate its credential-entry form on the token — an older daemon silently drops the body and forwards a body-less connect.
- **Isolated daemon mode for live E2E** (#333) — live daemon tests shared port 7533 and real user state, so they could not safely prove the current checkout. Hidden, paired test-harness flags `--isolated` / `--state-dir` / `--port` start a foreground daemon on a random port backed by a temporary absolute state directory, with Cloud WS, MCP, watchers, heartbeat, scheduler, sync, marketplace warming, memory services, and interrupted-turn auto-resume all suppressed. Isolated startup rejects `--detach`, `--force`, port 7533, relative or missing state directories, and Desktop RPC. The state override is a startup-only process hook, not an environment variable ordinary CLI commands could inherit. **The mode is state-isolated, not credential- or capability-isolated** — the OS credential store is process-global and the normal tool registry is retained, so live runs can bill the real API key and trigger real side effects. See `docs/live-e2e-testing.md`.

### Fixed

- **Force-stop could deliver nothing and record nothing** (#336) — the synthesis turn read only `OutputText` and silently swapped in the generic fallback line when it was empty, so an HTTP-success response whose visible text lived in `ContentBlocks`, or that was thinking-only (reasoning budget swallowing the answer on high-effort configs), lost the answer entirely. The path is now at parity with the normal final-response path: visible text is recovered from `ContentBlocks` before falling back, and a bare-empty synthesis retries **once** with reasoning stripped (`EffortTier=low`, thinking off) so the output budget goes to visible text. Sealed execution profiles (`kfp1`/`ep1`) own their configuration and keep single-attempt behavior; the recovery budget is explicit, capped at one, and independent of the main-loop empty-response retry. The response is persisted through `buildAssistantMessage` so thinking blocks survive, after hallucinated `tool_use` **and** native `computer_call` blocks are stripped — force-stop never executes calls, and `computer_call` sits off the ordinary tool_use family, so orphan-stripping alone could not sanitize it and an unpaired call broke next-turn replay. Every empty attempt and every non-cancellation recovery failure is audited content-free (`force_stop_empty_synthesis`, `force_stop_synthesis_recovery_failed`). First-attempt request configuration, `completeWithRetry`, and the normal final-response path are untouched.
- **Checkpoints missing the compacted-history marker were fed forward** (#334) — a structurally valid checkpoint without the marker cannot have been written by `ShapeHistory` (even a failed summary keeps the prefixed placeholder line), yet `HistoryForLoop` passed it on with only a warning and let next-turn sanitization silently degrade the primer. It is now treated like any other invalid checkpoint: log and fall back to the lossless archive, so the state self-heals on the next turn at the cost of one summary call.
- **TUI `/compact` diverged from the daemon's compaction pipeline** (#334) — the context window fell back to a hardcoded 128,000 instead of the live loop's auto-detected window (8x over-compaction on 1M-window sessions); `ForceShapeHistory` received `overheadTokens=0`, ignoring the estimator calibration every other compaction decision uses; and the shaped history carries only a placeholder system slot while the calibration is measured against requests that include the real prompt, so budgets treated the whole system prompt as free headroom. All three now match the loop, and the loop's mid-turn `CheckpointFunc` is wired in the TUI (2s debounce, matching the daemon) so an applied auto-compaction survives a crash before run end.
- **`/compact` and agent runs could write the same session concurrently** (#334) — slash dispatch precedes the inject branch, so `/compact` could start during an active run, and Enter during `/compact` fell through to the local-run path; either direction overwrote `m.cancelRun`, left the worker reading unsynchronized live-loop state, and let two checkpoint writers silently discard the paid manual summary. The two are now mutually exclusive through goroutine-lifetime guards: `compactInFlight` is released only by the done handler (the worker is still alive after Esc), `runInFlight` is a **counter** because Esc-then-Enter legitimately overlaps two run goroutines, and every run-starting door — plain submit, custom slash commands, `/research`, `/swarm` — goes through the shared refusal helper. `/compact` now runs under a cancel context assigned to `m.cancelRun` so Esc/Ctrl+C abort the LLM calls, results carry a generation stamp so a cancelled pass resolving under newer work is dropped, and all session mutation moved out of the worker into the done handler, pinned to the originating session id. A refused submit restores the composer input verbatim (including the raw `[Pasted text #N]` form) and re-adjusts the textarea height, so the try-again line is actionable instead of eating the turn.
- **Realtime SDP failures were undiagnosable** (#333) — a live full-path run took an HTTP 503 during SDP exchange with no provider correlation id to report. The failure now carries `request_id` when the response has one. Automatic retry was deliberately **not** added: the Realtime calls API exposes no idempotency contract, so replaying a failed POST cannot prove the provider did not already create a session.

### Changed

- **Koe's default realtime model is `gpt-realtime-2.1`** (#337), up from `gpt-realtime-2.1-mini`. The Standard/Advanced voice model picker is being removed from Kocoro Desktop, so the engine default becomes the model every voice call should run on rather than the cheaper tier — **this raises per-call voice cost for anyone on the default**. `--model` is unchanged and remains the escape hatch for CLI and on-robot callers, with its precedence now pinned by test. Live E2E exercises the shipped model too; the pre-release gate had been running a model nobody uses. Pairs with the corresponding Kocoro Desktop release.

## v0.4.2 — 2026-08-06 — Compaction reliability

**Context compaction becomes a durable, calibrated state transition.** Long sessions previously had three independent ways to degrade. The proactive trigger fired on real prompt usage while the shaping gates judged size with an uncalibrated `EstimateTokens` — blind to the tools[] schema mass and ~25% low on dense code — so sessions in the gap between the two measures compacted every turn, paid for the summary, discarded it, and then latched compaction off while the prompt grew unbounded (observed live 2026-08-04: a 130K prompt on a 60K window with zero effective compaction). The summarize call's own input was bounded in bytes on the assumption of ~3 bytes/token, so a transcript of JSON tool results overshot the small tier's window and returned a hard 400 — and since compaction is the only way an oversized session can shrink, every later turn failed identically. And the compacted result was recomputed per request rather than persisted, so the summary drifted each turn and the prompt cache never converged.

All three are closed, and the paths that had no recovery surface gained one: summaries are audited before acceptance, recently read files are restored after the history that held them is dropped, and the pre-compaction state is snapshotted to disk.

### Added

- **Estimator calibration persisted in session checkpoints** (#327) — the daemon builds a fresh `AgentLoop` per request, so a resumed session made its iteration-0 compaction decisions with zero calibration, exactly where the chars/3.5 estimate is least trustworthy. `(overhead_tokens, model, tools_fingerprint)` now rides the session at every save site that already carries the tool-result budget state, and is restored after `SetSessionID`. Restore discards samples that are non-positive, larger than the window, taken under a different tool registry, or incompatible with an active model pin.
- **Compaction summary quality validation** — `extractSummary` accepted any non-empty text, so a live run's single-line summary with no structure and no identifiers was accepted unvalidated and the identifiers in the dropped messages were lost permanently. `GenerateSummary` now audits its candidate at compaction scale: labeled section structure must be present, and identifier-shaped tokens scanned from the droppable middle (long hex, URLs, unix paths, `host:port`, long bare numbers; ≤12 unique) must appear verbatim. One retry carries the failure reasons; whatever it still misses is appended mechanically under `## Exact identifiers (auto-preserved)`. Short histories keep the single-call prose behavior.
- **Post-compaction file restoration** — shaping drops the middle of the history and with it the full text of files the model read there, while the content still sits on disk. After an applied **proactive or preflight** compaction the loop re-reads the most recently read files from the session `ReadTracker` and appends them as one system-reminder message ahead of the task reanchor: 5 files, ~5K tokens each, ~50K total, and the payload must keep the calibrated estimate under the trigger line. MEMORY.md/AGENT.md are excluded, reads surviving in the kept tail are skipped, and vision-format or non-UTF-8 files are never re-injected as bytes-as-text. The reactive path deliberately skips restoration — its 400-evidence floor is only a lower bound, and a second overflow there is terminal — and instead gets a reanchor variant warning that earlier file contents may be gone and must be re-read before quoting.
- **Sequential transcript fold** — head+tail truncation silently dropped the middle of very long transcripts before the summarizer ever saw it, so multi-day threads lost exactly the content compaction exists to preserve. Oversized transcripts are now split into chunks that each fit the small-tier input cap (boundaries never separate a `tool_use` from its `tool_result`) and folded into a running summary before the final structured pass. `maxSummaryFoldChunks` caps the spend with older chunks elided behind a marker; any failed or empty fold falls back to the previous rune-safe single call, so chunking can only add coverage.
- **Task-context micro-summaries** — the Tier-2 micro-compact prompt preserved errors, paths, and IDs but had no notion of what the agent was working on, so it could keep the wrong details. The current user request (head-clipped to 240 runes) is now threaded in as a `Current task` line; blank context leaves the prompt byte-identical.
- **Config reload state** (`config_reload_state_v1`) — the daemon records the exact global `config.yaml` revision reflected in memory. `GET /config` and `GET /config/status` report a newer external revision as `reload_required`, so an edit that has not been loaded is visible instead of silently ignored. Internal read-modify-write mutations preserve unknown external edits and never mark bytes they did not load as applied.
...
- **Compaction ran every turn and reclaimed nothing** — the proactive trigger judged real usage while `ShapeHistory`'s skip gate and the preflight guard judged an uncalibrated estimate, so any session in the `[90% real, 100% est]` band (few-turn, large-tool-result workloads: file reads, browser snapshots) triggered compaction every Run, paid `PersistLearnings` + `GenerateSummary`, discarded the summary, appended a false "Context was compacted" reanchor, and latched `compactionApplied` so both paths stayed disabled while the prompt grew unbounded. `AgentLoop.estOverheadTokens` now calibrates per response (real prompt tokens − `EstimateTokens(request)`) and every estimate-based decision adds it, so a mid-session model switch recalibrates within one turn. The proactive path latches and reanchors ONLY when history actually shrank; on a no-op the summary stays cached so the retry costs no extra call.
- **Back-to-back compactions** — aligning shaping acceptance to the same line as the trigger removed the dead zone but created churn: a shaped history landed just under the trigger, one large tool result re-crossed it, and the next iteration paid for a full summary and prompt-cache rebuild again (observed live: two full compactions within three iterations). Shaped candidates are now accepted only under the landing line while the trigger stays put, giving every compaction a hysteresis band before it can re-trigger.
...
- **A context-length rejection could not open the shaping gates** — a 400 proves the real prompt exceeds the window even when both the estimate and the calibration say otherwise (token-counting mismatch, or a resumed session that 400s before any usage sample exists this Run). The reactive path now floors its overhead at `window − estimate + 1`, so the retry actually carries a smaller history instead of resending the rejected one.
- **A successful reactive retry wiped the calibration** — the rebuild replaced `req.Messages` with the compacted history while the calibration denominator still pointed at the pre-compaction slice, so the retry's overhead computed negative and stored 0 — on exactly the session that had just proved it should be large. The denominator now follows every rebuild site.
- **TUI `/compact` reported failure while already running on the compacted view** — `runCompact` assigned `Session.CompactionCheckpoint` before persisting it, so a failed `Save()` told the user compaction failed while the in-memory session had already switched, and the next successful save persisted it anyway. The previous value is now restored on the error path, and `ArchiveThroughIndex` is captured next to the history snapshot it must agree with rather than relying on the state machine to hold across the summarize round-trip.
- **A failed summary dropped the conversation primer** — summarization failure or blank output left no compaction marker, so the next turn selected the wrong sanitizer and lost the original primer. A typed-looking marker is now always emitted and accounted for in forced TUI shaping.
- **Global skill deletion left per-agent references behind** (#324) — removal is now alias-aware and transactional across agents, dangling references are pruned at daemon startup, built-ins are protected, Cloud sync is serialized against deletion, and runtime listings distinguish per-agent enabled skills from the global inventory. Exact skill slugs take precedence over display-name aliases, corrupt manifests and builtin mutations return actionable wire errors, and cross-device attachment intent survives a skill that is not installed locally (startup pruning could previously turn device-local absence into a remote detach).

### Changed

- **Absolute compaction buffer on large-context windows** — the fractional 90%/80% trigger and landing lines forfeited ~100K usable tokens on the 1M-context families the default tiers now route to, exactly the case the Hardcoded Limit Policy calls out for re-checking on a model-family upgrade. The trigger is now `max(90% of window, window − 60K)`, the landing line `max(80% of window, window − 3×buffer)`, and the preflight backstop `max(95% of window, window − max(buffer/2, 32K))`. Windows under ~600K keep the fractional floors byte-identically. The landing line is three buffers wide because the band must hold the ~50K restoration payload plus more than one large turn pair, or a single moderate tool result re-arms the compaction just paid for.
...
- **Durable live-context checkpoints** — compaction is now a real state transition instead of a per-request view. `Session.Messages` and `MessageMeta` remain the lossless transcript for resume/search/share/sync, while daemon auto-compaction and TUI `/compact` persist a separate summary + retained-tail checkpoint bound to an exclusive raw archive index. Later turns reuse that stable prefix and append only the uncompacted archive tail, restoring prompt-cache convergence and preventing the summary from drifting on every turn. Rewind/truncate/reset invalidates the checkpoint, and mid-turn compaction preserves the complete current-run tool transcript before reshaping model-visible context.
- **Bounded compaction snapshots** — snapshots now protect replacement of the model-live checkpoint rather than duplicating a supposedly destructive transcript rewrite. Daemon auto-compaction and TUI `/compact` capture the exact prior live state; rollback JSON omits inline image payloads, defaults to one newest snapshot per session, and removes files older than 14 days during daemon startup. Raise `agent.compaction_snapshot_retention` for oldest-pinned multi-snapshot history; set `agent.compaction_snapshot_max_age_days: 0` to disable age cleanup.

## v0.4.1 — 2026-08-03 — Skill install recommendations + MCP hardening

### Added

- **Skill install recommendations (Desktop)** — signed-in Desktop requests declaring `skill_install_recommendation_v1` get `discover_installable_skills` and `offer_skill_installation`. Task-time discovery is binary-pinned and offline (embedded high-confidence allowlist only, never the GitHub/static/ClawHub registries); only after explicit acceptance do the document entries fetch their immutable SHA-256-verified archive. Offers are account+device directed, persisted with a 24h continuation token, and resume the original session with a hidden follow-up.

### Fixed

- **Recommendation cards lost across auth transitions** — the verified principal is read ONCE at the `/message` admission boundary and carried on the request; the `/events` sink binds lazily on the live stream with the bus subscription established BEFORE the bind, and card delivery resolves the live sink at emit time while pinned to the admitting epoch. Any principal transition (sign-out, account switch, same-account re-login) invalidates outstanding offers rather than misdelivering them. Content-free admission and registration log lines distinguish "tools never registered" from "model never called them".
- **MCP post-dispatch replay could duplicate writes** — a transport error after dispatch does not prove the server never acted, so replay is now gated on `readOnlyHint`/`idempotentHint`; everything else returns an explicit outcome-unknown result steering the model to verify before retrying. Supervisor-known-dead servers are probed BEFORE dispatch instead of discovering the corpse mid-call, and MCP-vs-MCP tool-name collisions resolve to a deterministic owner across registry rebuilds.
- **MCP config lost on reload** — `tool_timeout_secs` and `workspace_base` now propagate through `/config/reload` and survive the builtin-catalog merge. Disabled servers are no longer supervised, and probe-backoff overflow is clamped.
- **`PATCH /config` accepted unknown keys** — strict rejection via a reflection tree over `config.Config`, plus protected-field aliases.
- **`computer_use` display hotplug** — topology exhaustion and display hotplug recovery.
...
**Koe Fast/Full execution.** Voice-dispatched tasks now carry a soft semantic `execution_mode`: bounded work defaults to a reserved Fast profile (an opaque Cloud-minted `kfp1` id pinning the Luna route), while complex or high-risk work keeps the full global/per-agent Agent configuration byte-for-byte. The daemon re-decides admission on every request (missing or unknown modes fail closed to Full, `full_reason` can only keep Full — never upgrade), resolves Fast from Cloud with a 5-second bound, and degrades to Full on ANY resolution failure. The validated execution run (lineage ids + profile + digest-only evidence) is persisted per session, returned on `done` as `execution_run` for Koe's call ledger, and restored pinned across interrupted-turn recovery with exactly-once resume semantics.

> **Cross-repo contract:** capability tokens `koe_fast_profile_v1` and `agent_service_tier_v1` (WS handshake + `GET /status`); wire fixtures `message_koe_execution_{fast,full}_request.json` and `sse_event.done.with_execution_run.json`. The Fast route requires a Cloud deployment serving the `koe_fast` capability on `POST /v1/completions/resolve`; until that ships, every Fast request silently runs Full — functional but dark. Verify one live Fast request end-to-end (Luna model + fast tier observed) before relying on it.

> **Breaking.** Two changes affect existing setups and need a minor version bump, not a patch:
> 1. **Unattended legacy GUI automation now fails closed.** `computer`, `accessibility`, `applescript`, and `ghostty` joined `agent.DisallowsUnattendedAutoApproval`, reversing the earlier decision to keep existing schedules working. A schedule that drove AppleScript at 3am will now be denied. The replacement is the explicit global Computer Use grant (below), which `computer_use` honors on unattended runs.
> 2. **`accessibility` and `applescript` are removed from the model's toolset on daemon runs** where `computer_use` is registered, and `bash` rejects `osascript` / `cliclick` there, so a failed semantic path cannot bypass target binding and IME-safe insertion. TUI / one-shot CLI / MCP runs are unaffected and keep both tools.

> **Cross-repo contract:** five new **additive** capability tokens — `computer_use_topology_v1`, `computer_use_control_v1`, `computer_use_app_policy_v1`, `computer_use_physical_interference_v1`, `computer_use_risk_confirmation_v1` — on the WS handshake (`X-Kocoro-Capabilities`) and `GET /status`. Desktop must gate each surface on its token; when a token is present, transport/decode/endpoint errors are degraded fail-closed states, never a signal to fall back to legacy UI.
>
> The provider-native path depends on Cloud's `POST /v1/completions/resolve`, **verified live on 2026-07-25**: the served `gpt-5.6-sol` profile is semantically identical to the vendored `docs/desktop-wire-fixtures/execution-profiles-v1/profile.openai-native.json`, and the live bytes were replayed through the real consumer path (strict decode, duplicate-member rejection, `admitResolvedExecutionProfile`) yielding a trusted resolution that satisfies every condition of the OpenAI-native selection branch. Provider-native execution is therefore active, not dark.
>
> **Two Cloud contract changes to track:** continuation-token TTL is 24 hours (was 31 days), and an expired trajectory returns a new `computer_continuation_expired` (HTTP 409), distinct from a replay rejection (400). The daemon hardcodes no TTL assumption and correctly does **not** retry 409 (`isRetryableLLMError` covers only 429/5xx), so behavior is fail-closed — but it does not yet restart the trajectory either: the run surfaces the error and ends. Automatic restart is a follow-up.

### Added

- **Computer-use coordinator and Desktop control plane** (`internal/guicontrol/`, `internal/daemon/computer_use_*.go`) — one process-wide lease admits GUI-mutating workflows. A mutating tool waits for a real Desktop heartbeat before committing input; Pause / Take Over / Stop cancel the active action and hold an action-quiescence barrier until the executor acknowledges cleanup, so no second workflow can enter while the first can still commit. Local-presence-protected routes: `GET /local/computer-use/activity`, `POST /heartbeat`, `POST /control`, `GET /topology`, `GET`/`PUT`/`DELETE /app-policy`. Activity events (`computer_use.activity`) carry only structured, redacted state, ordered by `(coordinator_instance_id, revision)`.
- **Consequential-action confirmation** (`internal/daemon/computer_use_risk_broker.go`, `internal/tools/computer_use_risk_*.go`) — a separate one-shot local decision for send / delete / purchase intent, classified from bounded trusted AX metadata and never from model prose. The grant binds the complete canonical detail plus a target digest, never outlives its 30-second frame, and is invalidated by denial, cancellation, Stop, Take Over, re-observation drift, expiry, or turn end. A generic tool approval cannot satisfy it, and right/double click, drag, or keyboard actions cannot borrow it.
- **Provider-native execution profiles** (`internal/client/native_execution_profile.go`, `internal/agent/openai_computer_*.go`, `internal/tools/{openai,anthropic}_computer_adapter.go`) — a trusted Cloud-minted profile selects the Anthropic-native or OpenAI-native computer contract, or the generic AX-only function tool. Poisoned prior computer-call pairs are stripped from continuation requests; safety acknowledgements are one-shot capabilities bound to profile identity, response id, and a call digest, so they cannot be forged or replayed across calls or responses.
- **Global Computer Use permission** (`internal/daemon/alwaysallow.go`, `internal/agent/loop.go`) — one product-level grant across apps and agents, stored only as a global `permissions.always_allow_tools` entry. It is the single exception that authorizes `computer_use` on unattended runs; without it, scheduled / heartbeat / watcher / no-UI runs deny both observations and mutations, and a blanket `daemon.auto_approve` never substitutes for it.

- **Koe Fast/Full mode selection** (`internal/executionprofile/`, `internal/daemon/koe_execution*.go`, `internal/koe/`) — `DecideModeAdmission` over a closed vocabulary; call-scoped task lineages record validated execution runs so follow-up, cancel, and revision routing survive reconnects. `do_task` carries `execution_mode` + `full_reason`; the selector is deliberately soft, not a rigid classifier.
- **`agent.service_tier`** (`""`/`default`/`fast`) — process-global OpenAI processing lane for ordinary runs, validated in config and `PATCH /config`. Cleared for named-agent exact-model overrides; suppressed whenever a sealed execution profile (kfp1/ep1) owns the request.
- **`ask_user_question`** (`internal/daemon/question*.go`, `internal/tools/`) — closed-choice structured questions (1-4 questions, 2-4 options) over a request/resolve broker sharing `pendingCore` with approvals. Only sources with a real question UI (`questionUISources` allow-list: Desktop/web) get an asker; every other source — including Slack/Feishu/Lark/Teams/LINE, which have approval cards but no question transport — degrades to prose instead of blocking the run (#314). Capability token `question_v1`.
- **Projects** (#304) — session grouping with per-project theme colors, re-filing, and destructive-delete coverage. Capability tokens `session_projects_v1`, `project_entity_v1`.
- **ToolSearch explicit exposure + BM25** (#308) — per-tool `ToolExposure` overrides resolve independently from source defaults; deferred-tool retrieval ranks derived name/description/schema/namespace metadata with a deterministic BM25 index (max 8 ranked seeds before family expansion).
- **Bounded MCP tool calls** (#310) — every tools/call attempt is bounded by per-server `tool_timeout_secs` > `mcp.tool_timeout_secs` (default 300s); supervisor-known-dead servers are probed BEFORE dispatch; transport failures (classified by type, never by message text) probe fresh health and retry exactly once — timeouts and protocol errors never re-dispatch, so a completed side effect is never duplicated. Relative artifact links from path-aware servers (playwright built-in; user MCPs via `workspace_base`) are translated to absolute paths gated on existence + symlink-resolved containment; browser intermediates land in per-session scratch (`daemon.scratch_max_age_days` sweep, default 14d) instead of user-visible folders.

### Changed

- **Concurrent Computer Use recovery** — one exact task app now binds background-first, keeps semantic press/scroll and ordinary target-bound input off the user's foreground, and activates the target only when `foreground_allowed` encounters an action without an exact background primitive. Kocoro/controller and unrelated frontmost apps no longer replace the retained task target. Ambient physical interference keeps the lease active but requires one exact fresh observation before another mutation; explicit Pause/Take Over/Stop remains quiescent and user-owned. The activity payload shape and existing `computer_use_control_v1` state vocabulary are unchanged, so Desktop follows the authoritative `lease_state` / `action_phase` values without a new capability token.
- **Stateful macOS computer use** — the approval-gated `computer_use` workflow keeps stale-state protection, explicit screenshots, generic window reopen, tolerant integer argument decoding, bounded delay waits, and visible pointer movement. Unattended runs now require the explicit persisted global Computer Use grant described above; without it they are denied, observation actions included.
- **Legacy Accessibility mutations now prompt** — `accessibility` read-only actions remain approval-free, while `click`, `press`, `set_value`, and `scroll` require a `description` and attended approval. Unattended legacy automation is **no longer compatible** — see the breaking note above.
- **`applescript` required-field validation** — `Run()` rejects a missing/empty `script` or `description` with a `[validation error]` result (loop-detector early-stop preserved) instead of silently executing an empty script, matching `computer_use` and `accessibility`.
- **Per-agent always-allow scope** (`internal/agents/always_allow.go`, `api.go`) — `computer_use` is rejected at per-agent scope because the grant is global-only. The legacy GUI names are *not* rejected on write; they are dropped by `SanitizeAgentPermissionsConfig` instead, because they were legitimately persistable before joining `highRiskTools` and config writes are full-replace — rejecting would have made every affected agent permanently uneditable through the API.

- **Tool-call preambles** (#309) — model-authored progress text surfaces before tool execution, is constrained against synthesized filler, and resets for injected turns so an absorbed follow-up cannot inherit the prior turn's preamble.
- **`temperature` wire semantics unchanged** — the field keeps omit-when-zero: `agent.temperature: 0` (the default) continues to mean provider-default sampling on every channel. An execution-profile draft briefly made zero explicit on the wire; that was reverted before release and the contract is now pinned by test.

### Fixed

- **Stuck modifier keys after an unconfirmed release** (`internal/tools/axserver/Sources/InputCrashRecovery.swift`) — the commit gate quarantined the whole process on the first release it could not confirm, which also suppressed every *remaining* release. A two-modifier action whose Shift release could not be confirmed (for example because the user was physically holding Shift, since the check reads combined session state) left Command physically held down until the helper restarted. The quarantine now blocks new input only; releases are always posted.
- **Clipboard restored before the target consumed it** (`internal/tools/axserver/Sources/TargetBoundInput.swift`) — `type` reverted the pasteboard on a fixed 100 ms delay, so a busy target could service the synthetic Cmd+V afterwards and paste the user's *previous* clipboard content into the field — surfacing only as a value mismatch, and not at all when the target is sensitive and read-back is skipped. Restore is now deferred until after the verifier reads the target back.
- **`clipboard_touched` reported false after the pasteboard was cleared** (`internal/tools/axserver/Sources/TargetBoundInput.swift`) — both `type` preparation failures occur after `clearContents()`, leaving the clipboard empty or holding the text meant for the target, yet the acknowledgement claimed it was never touched.
- **Unredacted toolbar value in app context** (`internal/tools/axserver/Sources/AXHelpers.swift`) — the toolbar field is located by position, not by being a URL, so an app whose toolbar hosts a 2FA code, API key, or licence field surfaced that value as `context.url`. It was the only `AXValue` read in the helper without a sensitivity gate.
- **Unbounded AX calls during authority revalidation** (`internal/tools/axserver/Sources/TargetBoundInput.swift`) — `target_bound_input` was the only commit-adjacent path without a scoped messaging timeout, so an unresponsive target blocked the single-threaded socket loop past the commit horizon while Go's ack timeout released the GUI barrier — including while the user's pasteboard already held the agent's text.
- **Remote surfaces could mint the persistent Computer Use grant** (`internal/daemon/alwaysallow.go`) — "Always Allow" for `computer_use` from a Cloud-relayed channel or a paired device persisted the global grant, which also authorizes unattended runs; a tap in Slack permanently licensed 3am schedules to drive the Mac. Only a loopback surface may mint it now; remote taps allow the single call and emit `computer_use_grant_requires_local_presence`. This matches `POST /permissions/always-allow`, which already required `X-Kocoro-Local-Presence` for this tool.
- **Execution authority survived Stop / Take Over / lease expiry** (`internal/guicontrol/`) — those paths cancelled the action context but never revoked the minted capability, and the final execution gate is a pure capability check, so an action still inside classification or risk preflight could pass the gate. Input commit was backstopped one layer down, but the gate itself is now revoked explicitly and also honors context cancellation.
- **Take-over replay answered 409 for a take-over that succeeded** (`internal/guicontrol/coordinator.go`) — `AwaitActionQuiescence` treated a lease that had already ended as stale rather than trivially quiesced, so an idempotent retry after a lost response reported a conflict; a correct client reacts to 409 by *not* handing control back to the user.

- **Legacy Koe checkpoints abandoned on upgrade** (`internal/daemon/runner.go`) — a checkpoint written by a pre-execution-run daemon decodes a zero `ExecutionRun`, which the resume validator rejected; the first post-upgrade daemon start (the restart that triggers recovery) discarded the user's interrupted voice turn. Legacy checkpoints now mint a fresh Full run under the route lock — persisted into checkpoint AND ledger so a second crash still carries replay evidence; corrupt runs keep failing closed.
- **Computer-activation recovery vs async MCP startup** (`internal/agent/toolbudget.go`) — the checkpoint toolset fingerprint hashed the entire registry, but recovery runs while MCP servers are still connecting, so unrelated registry churn burned a recovery attempt on effectively every restart. The fingerprint is now scoped to the activation's own tool contract; genuine contract drift still fails closed.
- **Reactive compaction defeated on ordinary OpenAI Responses runs** (`internal/agent/loop.go`) — the post-compaction retry re-pinned `previous_response_id`, so the provider rebuilt its own un-compacted chain and the retry re-overflowed terminally. The cursor is dropped (full compacted transcript goes up) while the trajectory's exact model pin is kept.
- **Rejected execution run wedged the voice call** (`internal/koe/realtime.go`) — a daemon result whose run identity the ledger refused returned without terminal cleanup: `voice_state` pinned at "thinking", auto-muted mic never restored, call unresponsive until hangup. The rejected task is now marked failed and the busy state clears — gated on no other lane still running, so a parallel task's own completion keeps owning its transition.
- **Zero-tool hallucination nudge fired on plain conversation** (`internal/agent/loop.go`) — observational phrasing ("I can see", "the output shows") in a no-tool answer legitimately describes user-provided material; the nudge now requires a performed-action or live-GUI-surface claim.
- **Resolve-endpoint decoders hardened** (`internal/client/`) — the koe-fast and computer profile resolvers now share the ep1 resolver's bounded codec (64 KiB cap, duplicate-member rejection, unknown-field rejection), and in-loop computer resolves are bounded at 15s instead of riding the 600s HTTP client inside a phase the idle watchdog cannot see.
- **google-workspace / wedged-MCP incidents (2026-07-29)** — a tool call no longer blocks 6.5 minutes on a connection the supervisor marked dead hours earlier, and a lost playwright artifact no longer sends the model into a 242-second disk-wide `find /`.

### Notes

- The coordinate-mapping layer (multi-display Y-flip, rotated/mirrored displays, scale factors) has not had an independent review pass. Its frame binding — immutable frame id, 30-second expiry, topology generation, bounds, and pixel-center region matching — was spot-checked and fails closed, but the arithmetic itself warrants a dedicated review before this is relied on for unattended clicking.
- **`skip_cache_write` and `tool_choice` are accepted but inert on Cloud.** Both are relied on here: the post-turn prompt-suggestion fork sets `SkipCacheWrite` (documented as the guard that keeps the fork from fragmenting the prompt cache), and the memory preflight helper uses `ToolChoice` to force its tool call, degrading to a `no_tool_call` result when the model declines. Neither is a regression — Cloud dropped them silently before — but two documented invariants are not actually enforced end-to-end. This needs a Cloud-side request, not a daemon-side workaround.
- `v0.3.5` was tagged without a CHANGELOG section; it shipped the generic integrations broker proxy.
...
- **Session-grouped content search** ([#284](https://github.com/Kocoro-lab/Kocoro/pull/284), capability `search_v1`, `internal/session/search.go`, `internal/daemon/server.go`) — `GET /search?q=&scope=all|default|<agent>&limit=&offset=` returns one hit per matching session with a pre-segmented highlighted snippet, `match_count`, and `total`/`has_more` paging. Search runs over clean message text only — `tool_result`/`tool_use` dumps are excluded from both the FTS index and the returned snippet (schema bump to v6 forces a rebuild that purges prior tool-dump content). The query is a bound `MATCH ?` parameter (no injection); the agent slug is `ValidateAgentName`-checked before any path concat; `limit` is clamped to 100 and each scope's scan to 400 rows. Malformed FTS syntax maps to `400 invalid search query`, a lone broken per-agent index is tolerated when other scopes match. Title matching stays client-side (instant in-memory fuzzy match, merged with these content hits).
- **Byte-bounded remote session timeline** ([#282](https://github.com/Kocoro-lab/Kocoro/pull/282), capability `remote_session_timeline_v1`, `internal/daemon/remote_timeline.go`) — `GET /sessions/{id}?view=remote_timeline` returns a newest-first page projected for mobile: thinking/redacted-thinking dropped, images/documents → text placeholders, `tool_result` capped 4 KB, `tool_input` 8 KB, per-message hard cap 96 KB, whole page bounded to 768 KiB enforced on the final serialized bytes. Cursor paging (`before`) is an exclusive bound and never splits a tool-use/tool-result pair. The legacy `GET /sessions/{id}` remains the complete session.
- **ClawHub "hide installed" browse** ([#281](https://github.com/Kocoro-lab/Kocoro/pull/281), capability `clawhub_exclude_installed`, `internal/skills/marketplace.go`) — `GET /skills/clawhub?exclude_installed=true` drops already-installed skills and refills from later pages (bounded by `skills.marketplace.clawhub_exclude_fill_max_pages`, default 5) so the page stays populated. ClawHub-only; the frozen `/skills/marketplace/*` static-registry contract is untouched.
- **Device-local named-agent default cwd** ([#279](https://github.com/Kocoro-lab/Kocoro/pull/279), capability `agent_default_cwd_v1`, `internal/agents/`, `internal/daemon/agentsync.go`) — a named agent's `cwd` is validated at write time, surfaced as a non-fatal `Warnings` advisory (not a load failure) when the persisted path is unavailable on this device, and treated as device-local by cross-device agent sync: never uploaded, never overwritten or cleared by a pull (including legacy payloads that still carry it).

### Changed

- **ClawHub default-view resilience** ([#281](https://github.com/Kocoro-lab/Kocoro/pull/281), `internal/skills/marketplace.go`, `internal/config/config.go`) — a one-shot startup warm of the canonical default browse page (`skills.marketplace.clawhub_warm_on_startup`, default true) plus a view-agnostic last-good first-page fallback (served with `X-Cache-Stale: true` on a transient fetch failure) kill the intermittent "注册表不可达" 503 on the default marketplace tab. The retry helper now keeps the response body readable after it returns.
- **Skills `allowed-tools` accepts the list form** ([#283](https://github.com/Kocoro-lab/Kocoro/pull/283), `internal/skills/loader.go`) — frontmatter may declare `allowed-tools` as either the spec's scalar string (`Read Write Bash`) or a YAML list (`[Read, Write, Bash]`, common among ClawHub authors). Previously the list form failed to unmarshal, which 422'd the install and fail-open skipped the skill from `GET /skills`. On disk it is always re-serialized as the scalar form.

### Fixed

- **`file_edit` resilient to formatting drift** ([#280](https://github.com/Kocoro-lab/Kocoro/pull/280), `internal/tools/file_edit.go`, `internal/tools/file_edit_match.go`) — when an exact `old_string` match fails, a fuzzy fallback matches after folding typographic punctuation (smart quotes, en/em dashes, exotic spaces) and, failing that, line-by-line ignoring trailing whitespace / CRLF / then indentation. Fallbacks return exact byte spans (never a global `strings.Replace`), so the edit lands at the location actually matched; an exact match always wins, and an ambiguous fuzzy match still errors rather than replacing. A no-match now invalidates the file's read-dedup and appends a closest-line hint so the model can spot the real difference without a blind re-read.
- **Skills `allowed-tools` is fully fail-closed** ([#283](https://github.com/Kocoro-lab/Kocoro/pull/283), `internal/agent/loop.go`, `internal/tools/skill.go`, `internal/skills/`) — a present-but-empty allowlist (`allowed-tools: []`) now means "restrict to zero tools" (only the always-exempt `use_skill`/`think`/`tool_search`) instead of silently granting every tool. The execution-time filter gates on `!= nil`, not `len > 0`. This also closes a write-side round-trip hole: `WriteGlobalSkill` gated on `len > 0` and `yaml.v3` evaluates `omitempty` (via the zeroer interface) before `MarshalYAML`, so a zero-tool skill re-saved through `PUT /skills/{name}` (e.g. a description edit) dropped the key from disk and reloaded as unrestricted. `stringOrList.IsZero` now reports only a nil slice as zero and the write gates on `!= nil`, so the empty allowlist round-trips.

### Notes

- Follow-up (non-blocking): the remote-timeline projection relays `tool_use` inputs verbatim (≤ 8 KB) to the mobile client — thinking blocks and images are stripped, but a secret embedded in a `bash`/`http` tool call would flow over the Cloud relay. Consider redacting tool inputs before truncation. It matches what the localhost full-session view already exposes, but the remote view crosses a different trust boundary.

## v0.3.3 — 2026-07-13 — WeChat channel parity, koe answer discipline, memory fidelity
...
- **Per-agent `auto_approve` via the config API** ([#274](https://github.com/Kocoro-lab/Kocoro/pull/274), `internal/agents/api.go`) — the runtime already honored `AgentConfig.AutoApprove`, but `AgentConfigAPI` never surfaced it, so `PUT /agents/{name}` silently dropped it and `GET` never returned it — "skip all approvals" for a named agent was a no-op from any API client. Adds the tri-state `auto_approve` field (`nil` = inherit, omitted): `WriteAgentConfig` writes it when non-nil, `ToAPI` echoes it back so editors can hydrate it.
- **Opt-in preflight debug dump** (`SHANNON_PREFLIGHT_DUMP=1`, `internal/agent/preflight_dump.go`) — the `<private_memory>` block lives only in the in-flight user message and the audit row is content-free by design, so "did the answer model ever see fact X" incidents were unattributable. With the flag set, each injected block is appended verbatim as a `{timestamp, session_id, context}` JSONL row to `~/.shannon/logs/preflight_dump.jsonl` (file `0600`, dir `0700`). Off by default, best-effort (never fails the run it observes); the file holds private memory content — delete after debugging.

### Changed

- **WeChat (iLink) is now a first-class cloud channel** (`internal/daemon/runner.go`, `broadcast_gate.go`, `session_cwd.go`, `internal/prompt/builder.go`) — `wechat` joins `cloudSourceSet`: `plain` output (matches Cloud's raw-CDN-URL extractor — a markdown link would leave a `[name]()` shell after Cloud strips the URL), schedule-broadcast smart default, scratch CWD, skill filter, `@mention` handling, and `isValidScheduleSource`. The AI-facing and doc source lists are synced to the full `cloudSourceSet` so the model knows WeChat/Teams replies auto-return to the channel (also backfills previously-missing Teams/WeCom entries).
- **Per-group `evidence_tier` reaches the answer model** ([#274 follow-up], `internal/memory/types.go`, `internal/prompt/builder.go`, `internal/tools/memory.go`) — the sidecar emits a per-group `evidence_tier` (corroborated / singleton / derived / text), but `MemoryCandidateGroup` lacked the field, so the unmarshal→remarshal round-trip silently stripped it from both the `<private_memory>` preflight render and explicit `memory_recall` results, and the answer model flattened weak evidence into confident statements. Adds the additive field and extracts a shared `MemoryEvidenceGuidance` spliced byte-stably into the Memory & Retrieval prompt section: current user statements and verified observations take precedence, recorded values are preserved (never corrected from training knowledge), weaker tiers are qualified rather than dropped. The preflight render tags each record `[strength=<tier>]`; missing/unknown maps conservatively to `unrated`.

### Fixed

- **koe: route `do_task` by information nature, not a topic allowlist** ([#275](https://github.com/Kocoro-lab/Kocoro/pull/275), `cmd/koe.go`, `internal/koe/tools.go`) — the persona used a whitelist (only small talk, recap, trivial arithmetic answered directly) plus a blanket "your memory is unreliable" instruction, which pushed stable-knowledge questions ("how reinforcement learning works", Newton's laws) through `do_task` even though the model answers them well. Replaced with an information-nature criterion: answer directly from stable public knowledge; use `do_task` only when the answer depends on a real action, current/changing facts, the user's private data or system state, or a fact the model does not hold (time-anchored wording still routes to `do_task`).
- **koe: speak a safe status line for a partial run, not its progress tail** ([#276](https://github.com/Kocoro-lab/Kocoro/pull/276), `internal/koe/tools.go`, `internal/daemon/runner.go`) — a `do_task` run that soft-stops (idle/deadline timeout, iteration limit, force-stop) returns only the tail of whatever was streaming when it died, usually a progress line; the outcome mapper only special-cased `user_cancelled`, so every other partial voiced that fragment aloud as the finished result. `MapDoTaskOutcome` now returns a canned per-language "incomplete" line for any partial run and seeds no recap digest (cancel guard still runs first so a cancelled run stays silent). Adds voice-projection provenance to the daemon (`kind=authored|mechanical` + `partial` + `failure_code` per turn) so the authored rate is measurable.
- **daemon: canonical WeChat brand casing in session titles** ([#278](https://github.com/Kocoro-lab/Kocoro/pull/278), `internal/context/title_gen.go`) — `brandDisplayNames` lacked a `wechat` entry, so `SourceLabel("wechat")` fell through to upper-first → "Wechat"; titles now read "WeChat · …" for both the instant placeholder and the smart-title upgrade.
