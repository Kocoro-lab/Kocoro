...
| Schedule proactive push | `internal/daemon/scheduler.go` `broadcastReply` | After successful run, mirror heartbeat behavior: `SendProactive(agent, reply, sessionID)` so the agent's OAuth-bound Slack/Lark/Telegram channels receive the reply. Guards: non-nil WSClient, non-empty agent, non-empty reply, non-nil result. |
