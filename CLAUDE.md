...
**Per-request channel suppression** (`internal/daemon/skill_filter.go`): the `desktopOnlySkills` registry is filtered out of `loadedSkills` before any consumer sees it (use_skill tool registry, `AgentLoop.agentSkills` for listing + semantic discovery) when `isCloudSource(req.Source)` is true. Single producer-side filter in `runner.go` immediately after skill loading — keeps all three exposure layers above consistent. Only entry today is `kocoro-generative-ui` (its `html-artifact` fences only render in the Kocoro Desktop WKWebView host; raw HTML/CSS/JS on Feishu / Lark / WeCom / Slack / LINE / Telegram / webhook surfaces as a fenced code block). Drift test in `skill_filter_test.go` walks `desktopOnlySkills × cloudSourceSet` so adding a new cloud source or new desktop-only skill needs no manual channel-by-channel verification. The per-channel divergence lives in the scaffolded user message (already a non-cached position), not the cached system prompt — anyone later moving skill listing into the system prompt must revisit this filter to avoid cache fragmentation across sources.
...
| Built-in MCP catalog | `internal/mcp/builtins.go` + `internal/config/config.go mergeBuiltinMCPServers` | `BuiltinMCPServers` ships pre-bundled MCP servers (first entry: `intercom` via `npx mcp-remote https://mcp.intercom.com/mcp`) disabled by default. `config.Load` field-merges the Go-source catalog onto user yaml: command/args/type/url/context come from binary (auto-upgrade), disabled/env/keep_alive/connect_timeout_secs come from user yaml (preferences persist). Env merges key-by-key with user winning on conflicts so a future builtin can ship default env vars. PATCH /config refuses to mutate the daemon-owned fields on a built-in (`error: "builtin_mcp_immutable"`, 409). GET /config/status exposes a parallel `mcp_server_info` map with `{builtin, display_name, description, auth_hint, requires_auth?, authorized?}` so Desktop can render a toggle + OAuth confirm modal without hard-coding the catalog. `requires_auth: true` flags entries where activation triggers an out-of-process OAuth flow (Intercom: npx-spawn + browser tab, cold-cache 5–20s); `authorized` is the dynamic counterpart — `true` means daemon detected an existing token cache (`internal/mcp/MCPRemoteHasToken` for mcp-remote: md5(serverURL) + `~/.mcp-auth/mcp-remote-*/<hash>_tokens.json` + matching `_client_info.json`), so Desktop SHOULD skip the confirm modal on re-enable (mcp-remote will silently reuse the cached token, no browser opens, modal looks broken when activate button does nothing visible). |
| MCP async startup | `internal/mcp/client.go StartConnectAll` + `internal/tools/register.go RegisterAllWithBaselineAsync` | Daemon startup and `/config/reload` no longer block on MCP handshakes — they synchronously build the registry with local+gateway tools only, swap deps, then fire per-server connect goroutines through `StartConnectAll`. Per-server `inFlight` set (via `tryReserveInFlight`) dedups concurrent attempts: a reload that fires while the previous async connect is still inside Initialize/ListTools (Intercom OAuth window can be minutes) silently skips instead of spawning a duplicate subprocess that would crash on EADDRINUSE for the OAuth loopback port. `Reconnect` honors the same dedup so supervisor probes can't race against in-flight startup connects. Per-server timeout resolution: `MCPServerConfig.ConnectTimeoutSeconds` > `cfg.MCP.DefaultConnectTimeoutSecs` > 60s floor. Intercom ships with 300s so cold-cache `npx mcp-remote` plus user OAuth fits inside the window. On timeout the goroutine calls `c.Close()` in a side goroutine (mcp-go's `Close` blocks until inner Initialize/ListTools unwind, so we don't await it) which SIGTERMs the subprocess. Successful connects call `Supervisor.ProbeNow(name)` → state goes Healthy → OnChange rebuilds the registry, MCP tools appear live. Failed connects write an `mcp_connect` audit entry; the supervisor's periodic probes deliberately do NOT reconnect (user re-toggle OR `POST /config/reload` is the recovery path). |
| MCP subprocess reaping | `internal/mcp/processgroup_unix.go` + `internal/mcp/client.go cancellers` | Stdio MCP subprocesses spawn under their own process group (Setpgid=true) with a custom `cmd.Cancel` that signals `-pgid` SIGTERM + `cmd.WaitDelay=3s` SIGKILL backstop. Necessary because npx-bridged servers run as a chain (npx → npm exec → node mcp-remote): killing only the direct child leaves the grandchild holding the OAuth loopback port (e.g. mcp-remote on 14453), so subsequent toggle-on attempts crash with EADDRINUSE. ClientManager stores the per-server `cmdCancel` in `m.cancellers`; `Disconnect` / `Close` / `Reconnect` / inline transport-error reconnect ALL cancel the group before calling `c.Close()` (mcp-go's `Stdio.Close` runs `cmd.Wait()` and would block forever on subprocesses that ignore stdin EOF). |
...
| Session routing | `router.go` | `ComputeRouteKey` precedence: explicit `session:<id>` → thread → sender → agent → channel. Web/webhook/cron bypass (always fresh). **Schedule** named-agent runs share one session file regardless of Stateful, but when `Schedule.Stateful` is *false (default for new schedules) `runner.historySnapshotForRequest` returns an empty history to the LLM via `RunAgentRequest.OmitHistory`. Legacy schedules with `Stateful == nil` keep the pre-feature behaviour (full history). |
...
| Schedule proactive push | `internal/daemon/scheduler.go` `broadcastReply` + `internal/daemon/broadcast_gate.go` `shouldBroadcast` + `internal/daemon/client.go` `SendProactive` | After successful run: `broadcastReply(ws, sched, reply, sessionID)` consults `shouldBroadcast(sched)` before pushing. `shouldBroadcast`: explicit `Schedule.Broadcast *bool` override wins; otherwise smart default by `Schedule.CreatedFromSource` (IM source `slack/line/feishu/lark/wecom/telegram/webhook` → broadcast; else silent). Pre-feature schedules with empty `CreatedFromSource` → silent (safe). Daemon-side guards: non-nil WSClient, non-nil sched, non-empty reply. Empty `sched.Agent` is the default-agent path — Cloud's `ListByAgentName` uses `COALESCE(config->>'agent_name', '') = $3` to match channels whose binding stored no `agent_name` key. LLM tools `schedule_create` / `schedule_update` accept `broadcast: "auto"\|"on"\|"off"`; daemon HTTP `PATCH /schedules/{id}` mirrors. Capability token `schedule_broadcast_gate` advertises gate support to Desktop. |
...
| Memory client | `internal/memory/` | Daemon owns sidecar lifecycle + 24h bundle pull. `memory_recall` → `Service.Query` via UDS; falls back to `session_search` + MEMORY.md when not `Ready`. API key never on disk — only `sha256[:16]` fingerprint. **2026-05-22 schema-mismatch lockout signal**: `WaitReady` returns `*ReadyTimeoutError{LastCompatibility, LastSubCode, LastBundleVersion}`; sustained `incompatible_bundle/(no_manifest|version_out_of_range)` surfaces as `memory.reason=tlm_binary_too_old` with `detail.repair_needed{compatibility,sub_code,bundle_version}` on `GET /status`. Supervisor fires a one-shot `onIncompatible` self-heal (cloud-mode `puller.tick()`), then short-circuits to `StateDegraded` instead of burning the restart budget. `Sidecar.Shutdown` is idempotent (guards against double `exec.Cmd.Wait` via `cmdReaped` atomic); supervisor calls Shutdown on every failed-WaitReady child before re-Spawn so orphan tlm processes can't accumulate (production case: 5 zombies). Cold-start bootstrap pull fires when `current` is missing OR points at a bundle whose `manifest.json` is unparseable JSON (`currentBundleReadable`); JSON-valid-but-schema-newer manifests look readable here and route through the `onIncompatible` path instead. |
