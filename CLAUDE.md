...
- **Proactive compaction** triggers at 90% real usage: `PersistLearnings` → `GenerateSummary` (two-phase analysis→summary) → `ShapeHistory`. Shaped history must LAND under `compactRetargetFraction` (0.80) — the 90/80 gap is the hysteresis band that stops back-to-back compactions. A shaping no-op (nothing droppable) does not latch `compactionApplied` and keeps the summary cached for a free retry.
- **Pre-flight compaction** at 95% calibrated estimate (`shouldPreflightCompact`): backup gate before each main LLM call + force-stop turn. Emits `OnRunStatus("preflight_compaction")`; reuses the proactive path's cached summary when present.
...
- **Post-compaction file restoration**: after any APPLIED compaction, the loop re-reads the most recently read files (session `ReadTracker`) and appends them as one system-reminder user message before the task reanchor — 5 files / ~5K tokens each / ~50K total, and the payload must keep the calibrated estimate under the 90% trigger line (it may consume part of the hysteresis band). MEMORY.md/AGENT.md excluded; reads surviving in the kept tail skipped.
