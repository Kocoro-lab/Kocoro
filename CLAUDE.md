...
| Schedule proactive push | `internal/daemon/scheduler.go` `broadcastReply` + `internal/daemon/client.go` `SendProactive` | After successful run, mirror heartbeat behavior: `SendProactive(agent, reply, sessionID)` so the agent's OAuth-bound channels receive the reply. Empty agent is the default-agent path — Cloud's `ListByAgentName` uses `COALESCE(config->>'agent_name', '') = $3` to match channels whose binding stored no `agent_name` key. Daemon-side guards: non-nil WSClient, non-empty reply, non-nil result. NO empty-agent guard. |
