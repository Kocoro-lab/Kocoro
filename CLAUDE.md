...
**Qwen live vision** (`internal/koe/webrtc.go`): a body carrier can supply encoded H.264 constrained-baseline keyframes through `ConnectOptions.VideoSource`. Koe adds that video track only to Qwen before SDP negotiation and reads the source only while `CallActive` is true; prewarm, idle, and ended calls emit zero frames. Qwen vision travels as RTP media, never as an OpenAI `conversation.item.create` / `input_image` event. The body runtime must reuse its existing camera owner and return one independently decodable frame per configured interval.
