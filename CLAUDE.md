...
### Context Management
- **Model-aware context window** (`client.ResolveModelCapabilities` + `agent.ComputeEffectiveContextWindow`): auto-1M only via the specific-model path for Opus 4.6/4.7 and Sonnet 4.6 (prefix match); 200K for Sonnet 4.5/4 and Haiku 4.5; conservative 200K for unknown specific models (no tier fallback — guards against newly-released 200K-cap models silently widening). **Tier-only** resolution (`model_tier: medium / big / large` with no pinned `agent.model`) returns 200K conservatively for every recognized tier — Cloud-side priority/failover lands on non-auto-1M models (gpt-5.1 in large, sonnet-4-5 in medium failover) where speculating up to 1M would defeat preflight at the moment failover happens. Both `"big"` and `"large"` aliases accepted. Per-agent / `req.ModelOverride` re-routes via `loop.RefreshContextWindow`. **Ollama provider always disables auto** (`agent.EffectiveContextWindowAuto`) regardless of `agent.context_window_auto` — the Anthropic prefix table can't map local model names like `qwen3:8b`; honors the operator-configured static `agent.context_window` instead. No `anthropic-beta` header is sent (retired 2026-04-30; auto on capable models since 2026-03-13).
