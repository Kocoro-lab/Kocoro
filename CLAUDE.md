...
internal/
  ...
  koe/                   # ── VOICE FRONT-BRAIN (shan koe) ── cgo since audio.go
    ...
    dismissintent.go     #   isDismissPhrase/normalizeDismissPhrase: whole-utterance closed-vocabulary match (zh simp+trad / en / ja: 闭嘴/停/够了/退出/再见/bye/黙れ…) → deterministic hang-up, the RELIABLE half beside the model-judged end_call tool (gpt-realtime-mini called end_call for only 1/7 dismissals pre-persona-fix). While do_task is in flight, ambiguous stop words (stop/停/停止/やめて…) are left to the model so they can cancel the task instead of hanging up. KOE_DISMISS_DETECT=0 kills it, KOE_DISMISS_PHRASES extends. Tagless pure Go
