...
- **Cloud integrations** — connected services such as X are exposed to the local agent after Desktop refreshes the catalog. Read-only calls can run concurrently when Cloud explicitly marks them observational; older schemas remain conservative. Reconnect prompts name the exact entry under Kocoro Settings → MCP Servers.
- **X post handoff** — the deferred local `x_prepare_post` tool only builds an official X composer link. It never publishes or opens a browser; the user must click the link, review the draft on X, and click Post.
