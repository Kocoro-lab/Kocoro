...
- **Outbound integration writes prompt every call** — integration tools whose Cloud schema is marked `requires_approval` (e.g. `gmail_send_email`, X posting tools) cannot be persisted as Always Allow at any scope. The approval card hides the Always Allow button, the click (from an older client) is honored once but never saved, and an existing `always_allow_tools` entry for such a tool is ignored at runtime. Config self-heals too: agent config writes through the API drop such entries, and the next successful integration refresh prunes any that slipped in while the integration catalog was unavailable (e.g. during an API-key rotation). One click must never become a standing grant for unattended outbound sends; new provider write tools inherit this automatically from their Cloud schema.
