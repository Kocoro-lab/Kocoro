...
agent:
  ...
  observation_window_trigger:
    mode: hybrid                   # hybrid (default) | cold_cache | budget | every_turn | off
    aggregate_cap_runes: 0         # budget backstop; 0 derives a safe default from observation_window and the effective per-observation capture cap
    cold_cache_gap_minutes: 60     # idle gap before the provider prompt prefix is treated as cold; 0 uses 60
...
`hybrid` preserves a warm prompt prefix until accumulated browser/GUI text
crosses the budget, while also clearing for free after the configured idle gap.
`cold_cache` uses only the idle signal, `budget` uses only the size signal,
`every_turn` keeps the legacy per-iteration behavior, and `off` disables text
observation clearing. An explicit `aggregate_cap_runes` is honored verbatim;
the derived default includes enough room for four more observations after a
clear and follows `tools.browser_result_truncation` (or its
`tools.result_truncation` fallback).
