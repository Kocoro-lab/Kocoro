...
| Bundle ID 稳定 | `run.shannon.shanclaw`（ShanClawDesktop 现有 ID，**不要变**） | TCC 数据库按 bundle ID + code requirement 索引，ID 变了授权全清空 |
...
```swift
// ShanClawDesktop minimum target = macOS 15+（CLAUDE.md "macOS 15+ and iOS 18+"），
...
1. Desktop 当前是否已经持有稳定 Bundle ID + Team ID？v1 期间能冻结吗？实际 Bundle ID 为 `run.shannon.shanclaw`（来自 ShanClawDesktop CLAUDE.md "do not change"），不再用文档早期示例 `ai.kocoro.desktop`
...
5. ~~macOS 最低支持版本？~~ → 已确认 macOS 15+（ShanClawDesktop CLAUDE.md `macOS 15+ and iOS 18+`）。`requestFullAccessToEvents` 在 macOS 14+ 可用，本项目 always-on，文档已删去 pre-14 fallback 代码
