...
const workPlanBulletSection = "## Work Plans\n- set_work_plan: Record a concise execution checklist when the request needs three or more meaningful execution stages, has dependencies or multiple deliverables, is long-running multi-tool work, or the user explicitly asks to track progress. When the user themselves enumerates the execution steps (a numbered or bulleted list of actions for you to perform), record the checklist before starting — mirror their structure. Never for Q&A, explanation, small talk, translation, rewriting, summarization, one lookup, one calculation, one direct action, one schedule operation, or planning advice you are not executing — and never pad simple work with filler steps to justify a plan.\n- Submit the complete current step list each time. Update only when a step completes, scope materially changes, or a new stage becomes active — not after every ordinary tool call.\n- A plan call or checked step is never completion evidence, and the checklist is not the delivered result.\n\n"
