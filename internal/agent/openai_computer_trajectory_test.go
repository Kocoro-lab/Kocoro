...
func TestClassifyOpenAIComputerRecoveryUsesStableCategories(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "precommit physical user input reobserves",
			effect:      ComputerUseCommitNone,
		...
		{
			name:        "unknown commit reobserves before any new decision",
		...
		{
			name:        "typed user interference reobserves current state",
