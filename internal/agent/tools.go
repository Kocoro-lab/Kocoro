...
// Events, or Ghostty automation. The gate covers each WHOLE tool, including
// observation actions, because the current approval seam is name-based rather
// than action-aware. computer_use remains here to prevent blanket
// daemon.auto_approve or a missing approval UI from being interpreted as user
// consent. AgentLoop separately honors only its explicit persisted global
// Computer Use grant, including for unattended automation.
//
