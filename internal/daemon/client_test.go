...
func TestCapabilities_AdvertisesAgentResponseDetailV1(t *testing.T) {
	for _, capability := range Capabilities {
		if capability == CapAgentResponseDetailV1 {
			return
		}
	}
	t.Fatalf("default Capabilities = %v, want to contain %q", Capabilities, CapAgentResponseDetailV1)
}
...
func TestSendProactive_AllowsEmptyAgentName(t *testing.T) {
	...
	if err := c.SendProactive("", "hello from default", "sess-1", nil, nil); err != nil {
...
func TestSendProactive_StillDropsEmptyText(t *testing.T) {
	...
	if err := c.SendProactive("named-agent", "", "sess-2", nil, nil); err != nil {
		t.Fatalf("err: %v", err)
	}
	if called {
