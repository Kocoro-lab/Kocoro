...
func ToolDefs() []ToolDef {
	return []ToolDef{
		{Type: "function", Name: "do_task",
			Description: "do_task — how you actually get things done: your own hands on a full computer. As Kocoro on Kocoro Desktop you can browse and research the web, read and write files, run code and calculate precisely, manage schedules, send email and messages, and run multi-step jobs. Reach for it for ANYTHING beyond chat that needs real data, a current fact, an exact calculation, a real action, or content/results to show in Kocoro Desktop — never answer those from memory or guess. Long or multi-part spoken requests still count: preserve the user's details and do the task instead of waiting for a follow-up like \"do it\". The moment you call it, say exactly one short acknowledgement before the tool call: Chinese utterance -> \"我来处理\"; English utterance -> \"On it\". No second sentence and never both languages. Then speak the result in your own voice when it lands. It is you working with your own tools — never describe it to the user as a colleague, back-end, or other system. What comes back to you is a short spoken summary, not the full result; the complete report stays in the session and on Kocoro Desktop, so for a follow-up on a previous result, call do_task again referring to that earlier work rather than rebuilding it from the summary.",
