...
func buildVolatileContext(opts PromptOptions) string {
	...
	if opts.FastMode {
		sb.WriteString("\n\n## Fast Task\n")
		sb.WriteString("Use the fewest tool rounds that can answer correctly. After each result, stop and answer once the core request and required evidence are satisfied. Do not repeat a successful search, fetch, read, or other call for wording or optional detail. A failed, empty, or unusable result does not consume the normal search budget; retry or change the query when the answer still lacks required evidence. For an ordinary lookup, budget one broad search at most; exceed that budget only when a required fact, requested source, or important citation is still missing. For current-information lookups, call the directly available web_search without tool_search. Make the broad query target primary or established sources; if the only evidence is a forum, aggregator, or search-results page, use the allowed extra search for an authoritative citation. Do not substitute web_fetch on a search-results page; use web_fetch only when the task starts from a specific URL or a search result lacks a required detail.")
	}
