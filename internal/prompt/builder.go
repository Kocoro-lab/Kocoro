...
func macOSAutomationGuidance(toolNames []string) string {
	...
	if has("computer_use") {
		...
		bullets.WriteString("- `computer_use` cannot determine the device's physical location. Do not use shell commands, network lookups, public-IP geolocation, System Settings, Maps, or another app to infer it. Use a location the user already stated in the conversation; otherwise ask for a city or region.\n")
