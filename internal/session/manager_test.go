...
func TestManager_AppendAcceptedSpeculation(t *testing.T) {
	tmp := t.TempDir()
	mgr := NewManager(tmp)
	sess := mgr.NewSession()
	sess.Messages = []client.Message{
		{Role: "user", Content: client.NewTextContent("first turn")},
	...
	err := mgr.AppendAcceptedSpeculation(sess.ID, "run the test", "Running the test now...")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.Messages[2].Role != "user" || got.Messages[2].Content.Text() != "run the test" {
		t.Errorf("messages[2] = %+v", got.Messages[2])
	}
	if got.Messages[3].Role != "assistant" || got.Messages[3].Content.Text() != "Running the test now..." {
		t.Errorf("messages[3] = %+v", got.Messages[3])
	}
}

func TestManager_AppendAcceptedSpeculation_UnknownSession(t *testing.T) {
	tmp := t.TempDir()
	mgr := NewManager(tmp)
	err := mgr.AppendAcceptedSpeculation("no-such-session", "x", "y")
	if err == nil {
		t.Error("expected error on unknown session")
	}
}

func TestManager_AppendAcceptedSpeculation_SyncsCurrent(t *testing.T) {
	// Round 2 R5: if target is m.current, mutate in place so a later Save()
	// doesn't overwrite the append. Test by appending, then calling Save(),
	// then Load() — append must survive.
	tmp := t.TempDir()
	mgr := NewManager(tmp)
	sess := mgr.NewSession()
	sess.Messages = []client.Message{
	...
	if err := mgr.AppendAcceptedSpeculation(sess.ID, "next", "reply"); err != nil {
		t.Fatal(err)
	}
	// Call Save() — would overwrite if m.current wasn't synced
	if err := mgr.Save(); err != nil {
		t.Fatal(err)
	}
	...
	if len(got.Messages) != 3 {
		t.Errorf("after Save+Load, len=%d (want 3) — m.current not synced?", len(got.Messages))
	}
}
