...
- Response: `{"status": "updated"}`
- Notes: Supports `cwd`, `agent.model`, `agent.model_tier`, `agent.effort_tier`, `agent.response_detail`, `agent.language`, `agent.temperature`, `tools.allow`, `tools.deny`, `mcp_servers`, `permissions.always_allow_tools`. `agent.response_detail` accepts `concise` | `balanced` | `detailed`; omit or use `""` to inherit the global response style. `agent.model_tier` (one of `small` | `medium` | `large`) overrides the global `model_tier` for this agent only; omit to inherit. When both are set, `agent.model` (specific model id) wins over `agent.model_tier`. **`agent.model` must be a concrete model id (e.g. `claude-opus-4-8`), NEVER a tier word — to select a tier use `agent.model_tier`. A tier keyword (`small`/`medium`/`large`) in `agent.model` is rejected with HTTP 400; if it ever slipped through it would reach the Gateway as `specific_model` and fail every run with `model_id_unknown`.** Full precedence (highest first): `agent.model` (specific model id always wins) → `RunAgentRequest.ModelOverride` (heartbeat tier override) → `agent.model_tier` → local / project / global `model_tier` → viper default `"medium"`. `agent.language` locks the reply language (three-state: omit = inherit global; `""` = force mirror the user; native name like `日本語` = lock).
- `permissions.always_allow_tools` entries naming approval-required integration tools (Cloud schema `requires_approval`, e.g. `gmail_send_email`) are silently DROPPED by this full-replace write (response is still 200; ordinary entries are kept) — same drop-not-reject behavior applies to the full `PUT /agents/{name}` update when it carries a config. If the integration catalog does not currently list the tool (e.g. signed out, key rotation in progress), the entry is kept on disk but IGNORED at runtime, and the daemon prunes it automatically after the next successful catalog rebuild (`POST /integrations/refresh`, or a sign-in / account switch / key rotation). Never report such an entry as an effective grant. `computer_use` in this per-agent list is REJECTED with 400 on interactive config writes (its grant is global-only — use `POST /permissions/always-allow`); on the cloud agent-sync pull the same entry is silently dropped instead (nobody to report the error to), with ordinary entries kept.
...
- Agent sync is scoped to the signed-in account: an agent created or pulled under one Cloud account does not push into another account after a switch (device-local ownership; the agent stays usable locally either way). If a user asks why an agent is missing from another device/account, this is the likely cause.
...
- Response: `{"status": "added"}` on success; `400` if the tool is in `agent.DisallowsAutoApproval` (V1 GUI-control tools require fresh consent and cannot be persisted) or is a registered integration tool whose Cloud schema is marked `requires_approval` (outbound writes such as `gmail_send_email`).
...
- Default response: the complete persisted session record, including its full `messages` array. This historical response is unchanged and may be large. Additive optional fields ride along — notably `work_plan` (the durable per-run progress checklist, capability `work_plan_v1`; shape and recovery semantics in `events.md` under `work_plan.updated`), which the `remote_timeline` view below does NOT carry.
