...
- Method: GET
- Path: /agents
- Response: `{"agents": [{"name": "...", "display_name": "...", "builtin": false, "override": false}]}`
- Notes: Each entry includes `display_name`. When an agent has no display_name set, `display_name` falls back to the slug (`name`).
...
- **This endpoint REPLACES the whole agent config (PUT semantics).** To change a single field, GET /agents/{name} first, merge your field into the returned config, then PUT the full body — otherwise sibling fields (`cwd`, `tools`, `mcp_servers`, `permissions`, `watch`, `heartbeat`, other `agent.*`) are dropped.
