...
- `permissions.always_allow_tools` entries naming approval-required integration tools (Cloud schema `requires_approval`, e.g. `gmail_send_email`) are accepted by this full-replace write but IGNORED at runtime — the always-allow denial is enforced at dispatch, so never report such an entry as an effective grant.
