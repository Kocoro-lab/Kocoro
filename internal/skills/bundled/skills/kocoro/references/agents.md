...
- `permissions.always_allow_tools` entries naming approval-required integration tools (Cloud schema `requires_approval`, e.g. `gmail_send_email`) are silently DROPPED by this full-replace write (response is still 200; ordinary entries are kept) — same drop-not-reject behavior applies to the full `PUT /agents/{name}` update when it carries a config. If the integration catalog does not currently list the tool (e.g. signed out, key rotation in progress), the entry is kept on disk but IGNORED at runtime, and the daemon prunes it automatically after the next successful catalog rebuild (`POST /integrations/refresh`, or a sign-in / account switch / key rotation). Never report such an entry as an effective grant.
