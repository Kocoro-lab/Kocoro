...
| `agent.effort_tier` | **Unified reasoning-effort tier** (default unset). User-facing values: `low` (快速) / `high` (均衡) / `xhigh` (深思) / `max` (极致). Cloud translates the tier to each provider's native effort at request time (Anthropic passes it straight through; OpenAI maps `low→low` / `high→medium` / `xhigh→high` / `max→high`). Distinct from the OpenAI-native `agent.reasoning_effort` (Cloud prefers `effort_tier` when both are set, falling back to `reasoning_effort`). Per-agent override lives at `agent.effort_tier` in `~/.shannon/agents/<name>/config.yaml` (omit / `""` = inherit the global tier). A global change via PATCH /config needs `POST /config/reload` to take effect. | No |
| `koe.fast_effort` | **Voice fast-effort toggle** (bool, default ON) — when on, the Kocoro agent task triggered by voice is forced to the fastest tier (`low`) for low latency; this is NOT the realtime voice model (which has no effort knob). `null`/unset/`true` = force fast (default); `false` = do not override, the voice task keeps the agent's normal `agent.effort_tier`. Managed by Kocoro Desktop's Voice settings (a single toggle). | No |
...
| `daemon.auto_approve` | Skip approval prompts except for tools on the unattended deny-list (`computer_use` as of 2026-07-22) | No |
...
| `permissions.always_allow_tools` | **Tool-level approval bypass** (global scope, applies to every agent). List of tool names whose approval prompt is skipped — e.g. `[bash, file_write, http]`. Companion per-agent field lives at `~/.shannon/agents/<name>/config.yaml permissions.always_allow_tools`; the runtime unions both. Persistence is gated by `agent.DisallowsAutoApproval` (currently empty as of 2026-05-18). On unattended/no-broker runs, entries in `agent.DisallowsUnattendedAutoApproval` (`computer_use` as of 2026-07-22) ignore this bypass. High-risk bash command prefixes (`pip install`, `rm -rf`, `python -c`, etc.) still prompt every call regardless. Endpoints: `POST/DELETE /permissions/always-allow` (global), `POST/DELETE /agents/{name}/permissions/always-allow` (per-agent). | No |
