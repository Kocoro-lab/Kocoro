...
| `model_tier` | Global LLM tier (default `medium`). One of `small` / `medium` / `large`. Per-agent override is available via `agent.model_tier` in `~/.shannon/agents/<name>/config.yaml`. Precedence (highest first): `agent.model` (specific model id always wins) → `RunAgentRequest.ModelOverride` (heartbeat tier override) → `agent.model_tier` → local `config.yaml model_tier` → project `config.yaml model_tier` → global `config.yaml model_tier` → viper default `"medium"`. | No |
