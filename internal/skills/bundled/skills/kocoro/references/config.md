...
| `koe.audio_processing` | Voice microphone processing mode: `auto` (default), `mac_voice` (use Apple VoiceProcessingIO voice processing/AEC), or `clean_device` (for microphones/apps that already clean voice; keep VPIO device binding/playback but bypass Apple's voice processing). Kocoro Desktop exposes this under Voice → Advanced and forwards it to `shan koe --audio-processing`. | No |
