...
| `agent.compaction_snapshot_retention` | How many pre-compaction history snapshots to keep per session under `<sessions-dir>/.compaction-snapshots/<session-id>/`. Each applied compaction irreversibly replaces the session history; the snapshot is the rollback material. The oldest snapshot is pinned (pre-first-compaction copy) and rotation evicts from the second-oldest up. Deleting a session removes its snapshots. Default: 3; `0` disables snapshotting. | No |
