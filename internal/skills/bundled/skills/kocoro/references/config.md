...
| `agent.context_window_auto` | When `true` (default), resolve the active model's true context window. The 1,000,000-token cap is only returned when `agent.model` is pinned to a known auto-1M model name (`claude-opus-4-6`, `claude-opus-4-7`, `claude-sonnet-4-6`, including dated variants). Tier-only configurations (`model_tier: medium / big / large`) return the conservative 200,000-token floor — Cloud-side priority/failover can land on non-auto-1M models (e.g. `claude-sonnet-4-5`, `gpt-5.1`) where speculating up to 1M would defeat preflight compaction. **Ollama provider always disables auto-resolution** regardless of this flag (the Anthropic prefix table can't map local model names like `qwen3:8b`). When `false` or for Ollama, honor the static `agent.context_window`. Per-agent overrides take precedence over both. | No |
