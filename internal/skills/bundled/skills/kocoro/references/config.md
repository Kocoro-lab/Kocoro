...
| `agent.response_detail` | **Provider-neutral visible-answer detail** (`concise` / `balanced` / `detailed`, default `balanced`). This is separate from reasoning effort and is rendered into the cacheable BP3 user context; it is not forwarded as an OpenAI/Anthropic request parameter. A user's explicit request for response length, structure, or format wins. Per-agent override lives at `agent.response_detail` in `~/.shannon/agents/<name>/config.yaml` (omit / `""` = inherit the global profile). Requires `agent_response_detail_v1`; a global PATCH needs `POST /config/reload`. | No |
...
| `koe.provider` | Realtime voice provider routing: `auto` (default; OpenAI primary, Qwen only on a classified pre-media bootstrap failure), `openai`, or `qwen` (forced modes never fall back). PATCH rejects other values with 400. Gated by the `koe_realtime_provider_v1` capability token. | No |
| `koe.qwen_model` / `koe.qwen_voice` | Qwen Realtime model and voice ids, separate from the OpenAI `koe.model`/`koe.voice`. PATCH validates all four against the `realtime_catalog` allowlists and rejects unknown values with 400 (empty = provider default). | No |
| `koe.fast_effort` | **Fast voice-task profile** (bool, default ON) — when enabled, every new Koe task requests Fast; Realtime does not classify or choose the execution mode. The daemon resolves the trusted Fast profile through Cloud and sends only its opaque id, so the caller cannot choose a model. Setting this field off, resolver/capability failure, or a daemon-validated inherited Full lineage falls back to the complete normal global/per-agent configuration. This does not change the Realtime voice model and does not force normal Agent effort to `low`. | No |
...
| `agent.max_iterations` | Emergency model/tool-round fuse per turn (default 256; normal completion is outcome-driven) | No |
...
| `agent.run_event_retention` | Maximum observation-only run attempts retained per session under `<sessions-dir>/.run-events/<session-id>/`. Opening a newer attempt prunes the oldest attempt's JSONL, incomplete marker, and lock; transcript, audit, and recovery state are separate. Default: 32; `0` disables count pruning. | No |
| `agent.run_event_max_age_days` | Maximum age for run-event JSONL, incomplete markers, and lock files. The daemon sweeps the default and every named-agent session scope once at startup; active locked attempts are skipped. Default: 14 days; `0` disables age cleanup. | No |
