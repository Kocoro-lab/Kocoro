...
- Method: GET
- Path: /config/status
- Response: `{"mcp_servers": {"slack": "connected"|"enabled"|"disabled"}, "koe": {"enabled": bool, "model": "...", "voice": "...", "agent": "...", "language": "...", "audio_processing": "auto"|"mac_voice"|"clean_device", "effort_tier": "low"|"high"|"xhigh"|"max"}}`
...
| `koe.effort_tier` | **Voice Task Effort** — the reasoning-effort tier for the Kocoro agent task triggered by voice (not the realtime voice model, which has no effort knob). Same value domain as `agent.effort_tier` (`low`/`high`/`xhigh`/`max`). Empty defaults to `low` at injection to keep voice low-latency, independent of the agent's text-mode effort. Managed by Kocoro Desktop's Voice settings. | No |
