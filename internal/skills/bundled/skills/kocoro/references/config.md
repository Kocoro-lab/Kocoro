...
| `agent.compaction_snapshot_retention` | How many pre-compaction history snapshots to keep per session under `<sessions-dir>/.compaction-snapshots/<session-id>/`. Inline image blocks are replaced by text markers before the synchronous JSON write; snapshots retain text, identifiers, tool evidence, and decisions without screenshot base64. With retention ≥ 2 the oldest snapshot is pinned and rotation evicts from the second-oldest up; retention `1` keeps only the NEWEST snapshot. Deleting a session removes its snapshots. Default: 1; `0` disables snapshotting. | No |
