...
| `computer_use.activity` | Redacted, revisioned state of the single process-wide GUI-control lease. Payload schema v1 identifies the coordinator instance, lease/session/action/tool-use IDs, target app bundle/name, action phase/result/path, optional logical-point pointer, stable failure code, and timestamp. Desktop repairs revision gaps from the authoritative local snapshot before rendering controls. No typed text, AX values, window titles, screenshots, or model-authored descriptions are emitted. See `computer-use.md`. | Bus |
...
| `agent_reply` | Agent finished a turn (WS / schedule / Cloud sources). | Bus |
| `agent_error` | Agent run failed. | Bus |
| `notification` | Agent-authored `notify` tool call. Payload `{session_id, agent, source, title, body, sound}`. Automatic reply completion is not emitted as this event: a client that accepts `agent_reply` owns completion presentation; only a detached macOS run uses a direct local banner fallback. This keeps automatic completion distinct from agent intent and preserves partial metadata on the authoritative reply event. | Bus |
| `approval_request` | Tool needs user approval; payload `{request_id, session_id, agent, tool, title, source, channel, args, flags?, ts}`. `title` is parsed from `args.description` (falls back to `tool`). `args` is redacted+truncated (event-bus copy only; the wire copy sent to Cloud stays unredacted so Slack/etc. can render the real command). `flags` carries policy hints. `always_allow_disabled` is emitted for tools whose grants cannot be persisted: the legacy GUI-control tools (`computer`, `accessibility`, `applescript`, `ghostty`) and integration tools whose Cloud schema carries `requires_approval` (material outbound writes such as `gmail_send_email` or X posts — one click must never become a standing unattended grant). `computer_use` deliberately omits that flag: selecting Always Allow creates the single global Computer Use grant. Emitted only after the outgoing request reached the transport successfully — failed sends never publish a card. | Bus |
...
| `schedule_run` | Scheduled agent run lifecycle; payload `{schedule_id, session_id, agent, phase, error?, usage?, partial?, failure_code?, ts}` where `phase` ∈ started / succeeded / partial / failed. `partial` carries a useful but incomplete result and includes its stable `failure_code`; it must not be presented as clean success. `session_id` is empty for `started` and for failures before session resolution. `error` is present only on `failed`. Usage is retained on any terminal run that reached an LLM call and omitted when zero. Two events fire: `started`, then exactly one terminal event. Capability: `schedule_run_partial_v1`. | Bus |
| `session_title_updated` | The async smart-title upgrade (`fireTitleAfterRun`) persisted a new title for a session — including background scheduler runs the user's open window never triggered a re-list for. Payload `{session_id, title}`. UI clients update the matching session-list row in place so the title stops lagging the first-line/route placeholder until the next manual `GET /sessions`. Additive — older clients that don't decode it keep working. | Bus |
| `deliverable` | The `present_deliverable` tool surfaced a finished artifact (PDF/PPTX/XLSX/DOCX/HTML/image/…) for the user's Deliverables sidebar. Payload `{session_id, agent, source, id, path, filename, title, mime, byte_size, ts}`. `path` is **daemon-validated before emission** as a real regular local file resolved from an absolute, `~`, or session-CWD-relative path, but it is **not confined to the session working directory**. Clients may trust that the metadata came from the daemon and that the path existed as a regular file at emit time; they must not treat it as sandbox authorization, proof that the agent authored the file, or permission to upload/share the file without a separate user action. The model-authored tool args never reach the client. `id` is a stable `dlv_<hex>` handle. Desktop renders it as an open/preview card in the session's Deliverables sidebar. The call also persists as the `present_deliverable` tool_use/tool_result pair; the tool_result is daemon-authored pure JSON shaped as `{delivered, deliverable:{id,path,filename,title,mime,byte_size}}`, so the deliverable re-surfaces when the session is reopened even if no client was attached at emit time — and it stays in the `/events?since=` replay ring. Because the same `id` can therefore reach a client via live SSE, replay, and persisted history, **clients must dedupe by `id`**. Additive — older clients that don't decode it keep working. | Bus |
...
| `question.request` | The `ask_user_question` tool is asking the user to pick from enumerated options (second request/resolve interaction after approvals — dotted domain family). Payload `{request_id, session_id, agent, source, channel, questions, auto_resolution_ms?, ts}`. `request_id` is `qst_<hex>`. `questions` is a 1-4 array of `{id, header, question, multi_select, allow_other, options:[{label, description, preview?, recommended?}]}` (`id` is `q0`..). `auto_resolution_ms` (omitted when 0) is the clamped auto-decline window; when it is 0 the daemon falls back to the 5-minute `ApprovalTimeout`. Per-request SSE carries the same struct under the shorter name `question`. Capability token `question_v1`. **Source-scoped**: only runs whose source has a question UI (Desktop-local — `desktop`/`kocoro`/`shanclaw`/`web`) ever emit this. Messaging channels never do, including Slack/Feishu/Lark/Teams/LINE despite their Allow/Deny approval cards, because there is no Cloud transport for questions; those runs get a "can't ask here" tool result instead. Do not design around a question round-trip on a channel. | Bus + per-request |
| `question.resolved` | Terminal event that dismisses the question card; payload `{request_id, action, resolved_by, ts}` where `action` ∈ answer / decline / cancel and `resolved_by` ∈ kocoro (a UI answered via `POST /question`) / daemon (synthetic cleanup on timeout / ctx cancel / disconnect, always `action: "cancel"`). Same at-most-one-terminal-event guarantee per `request_id` as `approval_resolved`. | Bus |
| `auth_state_changed` | Email/password auth state transitions (macOS-only `/local/auth/*` endpoints). Payload `{state, user?, pending_email?, last_error_code?, updated_at}` where `state` ∈ `signed_out` / `pending_verification` / `logging_in` / `bootstrapping_key` / `signed_in`. See `auth.md` for the full state machine. Emitted only on state changes (suppressed when from==to). | Bus |
| `work_plan.updated` | Full latest work-plan snapshot for a session — the durable progress checklist recorded by the `set_work_plan` tool (dotted domain family, capability `work_plan_v1`). Payload `{session_id, run_id, plan_id, revision, lifecycle, close_reason?, explanation?, steps:[{content, status}], completed, total, updated_at, ts}` where `lifecycle` ∈ active / completed / stopped and `status` ∈ pending / in_progress / completed. Emitted **only after the snapshot's durable save succeeded**, so a displayed revision cannot vanish in a daemon crash. Full snapshot + monotonic revision is the recovery contract: clients drop lower-or-equal revisions of the same `plan_id`, may coalesce under backpressure, and refetch `GET /sessions/{id}` (whose `work_plan` field is recovery authority; the `?view=remote_timeline` projection does NOT carry it — use the default detail) after a gap or reconnect. Lifecycle closure is runtime-owned (`LastRunStatus` evidence, never a model claim) and bumps the revision. Plan state must never drive the outer task lifecycle. Additive — older clients that don't decode it keep working. | Bus |
...
| `upstream_inconsistent_finish` | Upstream returned `stop_reason=tool_use` but no `tool_use` content block (and no visible text). Daemon retries the same request inline (no outer iteration consumed; no top-of-loop compaction/drain runs between attempts); if the retry budget exhausts, the run falls back to the standard `empty_final_response` audit + `CodeEmptyResponse` friendly text. Detail is an English fallback (`"upstream returned an incomplete response; retrying"`); clients should localize off the code, not the string. Audit attribution: look for `event=empty_with_inconsistent_finish` rows. |
...
| `compaction_started` | A compaction pass began; detail is the phase (`proactive`/`preflight`/`reactive`/`force_stop`). TUI `/compact` does not emit these (it renders its own local progress). UI clients may show a transient "tidying context" indicator; gate on the `compaction_status_events_v1` capability to distinguish an older daemon from a run that never compacted. |
| `compaction_finished` | The compaction pass ended (applied, no-op, or failed alike — always pairs with `compaction_started`). Gated by `compaction_status_events_v1`. UI clients remove the indicator; any later session event should also clear it as a backstop. |
...
{ "schedule_id": "sch_1", "session_id": "sess_99", "agent": "bot", "phase": "succeeded", "usage": {"input_tokens": 612, "output_tokens": 56, "total_tokens": 668, "cost_usd": 0.0021}, "ts": "2026-05-15T09:00:42Z" }
{ "schedule_id": "sch_1", "session_id": "sess_99", "agent": "bot", "phase": "partial", "partial": true, "failure_code": "iteration_limit", "usage": {"input_tokens": 612, "output_tokens": 56, "total_tokens": 668, "cost_usd": 0.0021}, "ts": "2026-05-15T09:00:42Z" }
...
- `schedule_run` partial terminals are advertised by `schedule_run_partial_v1`; older clients ignore the unknown `partial` phase instead of rendering it as success.
...
One structural note for request/resolve interactions: `approval_request`/`approval_resolved` and `question.request`/`question.resolved` are the two request-lifecycle interactions today. They share ONE pending-interaction core (`internal/daemon/pending.go` `pendingCore`) that encodes the hard-won race-safety invariants (at-most-one terminal event, bus-ID ordering); `ApprovalBroker` and `QuestionBroker` are its two wire faces. A third interaction kind (device action approvals, …) must build on `pendingCore` too — do not copy a broker.
