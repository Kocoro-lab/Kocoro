...
| `notification` | Two triggers: (1) agent-authored `notify` tool call; (2) daemon reply-complete banner — fires once per `agent_reply` for interactive sources (skipped for cloud-distributed channels: slack/line/feishu/lark/telegram/webhook, and skipped when the reply text is empty). Both paths route through `tools.SendBanner`, so the Desktop-handler-or-osascript-fallback contract below applies identically. Payload shape: `{session_id, agent, source, title, body, sound}` (`body` is redact+truncated). | Bus |
