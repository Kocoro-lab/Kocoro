...
| `message_received` | User message was accepted for a session. Normal run-start messages are already persisted when this fires. Active-run follow-ups injected through the daemon WS path also emit this event with `queued: true` so clients can show a read-only "waiting for current turn" card until the transcript catches up. Payload `{agent, source, sender, session_id, message_id, text, queued?}`. Older daemons omit `queued`; treat missing as false. | Bus |
