...
| `upstream_inconsistent_finish` | Upstream returned `stop_reason=tool_use` but no `tool_use` content block (and no visible text). Daemon retries the iteration once with the same request; if the retry exhausts, the run falls back to the standard `empty_final_response` audit + `CodeEmptyResponse` friendly text. Detail is user-facing Chinese copy ("上游响应不完整,正在重试…"). Audit attribution: look for `event=empty_with_inconsistent_finish` rows. |
