...
- Body: `{"tool": "bash"}`
- Response: `{"status": "added"}` on success; `400` for legacy GUI-control tools in `agent.DisallowsAutoApproval` and for registered integration tools whose Cloud schema is marked `requires_approval` (outbound writes such as `gmail_send_email`). `computer_use` is accepted.
...
- **Token-prefix family matching is implicit**: Even without `*`, an entry like `terraform workspace list` automatically covers `terraform workspace show`, `terraform workspace select`, etc. — same exec, same first N tokens. Different sub-commands of the same exec (e.g. `git status` vs `git push`) do NOT auto-match. Use this to keep `allowed_commands` short.
