...
- `POST /uploads` — multipart **image** upload, used for agent avatars. Send a `file` part (`png` / `jpeg` / `webp`, body capped at 10 MiB; Desktop caps at 2 MiB) plus an optional `content_type` field; returns `{"url": "https://static.kocoro.ai/...", "size", "content_type", ...}`. Proxies to the cloud's **ephemeral** endpoint (`POST /api/v1/uploads/ephemeral`), so unlike the library uploads here the file is **NOT** recorded in `user_uploads` — it never appears in `GET /uploads` / the Published Files panel and cannot be retracted (avatars are referenced by URL, not user-managed files). Unsupported MIME → 400; over the cap → 413; cloud not configured → 503. Pair the returned URL with `PUT /agents/{name}` `avatar` to set an agent's avatar.
- `DELETE /uploads/{id}` — same response shape and 404 semantics. Owner-only.

Both endpoints transparently proxy the request through Shannon Cloud using the daemon's configured `api_key`. Desktop builds its UI on these — the LLM tools and the management panel share the same backing data. Session-share uploads (POST `/sessions/{id}/share`, sync or async) automatically tag `kind=session_share` and stash `{session_id, agent?, title?}` in `metadata`; `title` is the trimmed share-time session title, preserved as Unicode and capped at 256 runes. Blank titles and the default agent omit their optional keys. `publish_to_web` tags everything `kind=other`.
