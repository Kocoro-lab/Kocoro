...
- **Runs without interaction**: Scheduled tasks execute automatically and unattended. The agent will use tools without asking for approval — except tools on the unattended deny-list (`computer_use` as of 2026-07-22): those are never auto-approved on scheduled runs, even if the user persisted an "Always Allow" for them, so a schedule cannot use `computer_use` to drive the GUI with nobody present. Make sure your prompt is specific enough that the agent knows what to do without needing clarification, and don't build schedules around `computer_use`. For patch compatibility, legacy `accessibility`, `computer`, and `applescript` remain eligible for unattended approval; existing schedules using them still work and retain that risk.
