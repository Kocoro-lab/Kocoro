...
- **Runs without interaction**: Scheduled tasks execute automatically and unattended. `computer_use` may drive the active Mac session only after the user explicitly enables the global **Always Allow Computer Use** permission; without that persisted grant, observations and mutations both fail closed. Blanket `daemon.auto_approve` is not sufficient. Legacy GUI wrappers (`computer`, `accessibility`, `applescript`, `ghostty`) remain unavailable to unattended runs. Built-in sensitive-app blocks, point-of-risk confirmation, lease/Stop/Take Over, and physical-interference handling still apply.
...
- **Output destinations**: Runs with a usable reply, including explicitly marked partial results, are delivered to (a) the local session JSON, (b) Kocoro Desktop via `schedule_run`, and (c) **the IM channel the schedule was created in** (Slack / Teams / Lark / Feishu / LINE), gated by `broadcast`. A schedule can never deliver to any other channel: the originating channel's routing context is snapshotted at creation time and is the only legitimate IM target.
