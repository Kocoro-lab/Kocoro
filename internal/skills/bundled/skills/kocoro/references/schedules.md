...
- Notes: `agent` is optional — omit to use the default agent. `cron` uses standard 5-field cron format. `stateful` is the single "remember across runs" switch and defaults to `false`: each run starts in a brand-new session with no prior context (recommended for digest/polling/PR-review style tasks). Set `stateful: true` to make the schedule accumulate in one dedicated session and have each run see prior runs' history (continuous tracking, a rolling standup/journal). Applies to both the default and named agents. See the Stateful note below. The body also accepts `broadcast` (`"auto"|"on"|"off"`), but HTTP-created schedules cannot carry an origin-channel snapshot, so they NEVER deliver to IM — `broadcast:"on"` is stored but inert (the run logs a skip). To get IM delivery, create the schedule with `schedule_create` from a conversation in the target channel.
...
- **Runs without interaction**: Scheduled tasks execute automatically and unattended. The agent will use tools without asking for approval — except tools on the unattended deny-list (`computer_use` as of 2026-07-22): those are never allowed on scheduled runs — not even their approval-free observation actions like `screenshot`, and not via a persisted "Always Allow" — so a schedule cannot use `computer_use` to drive or observe the GUI with nobody present. Make sure your prompt is specific enough that the agent knows what to do without needing clarification, and don't build schedules around `computer_use`. For patch compatibility, legacy `accessibility`, `computer`, and `applescript` remain eligible for unattended approval; existing schedules using them still work and retain that risk.
...
- **Output destinations**: Successful runs are delivered to (a) the local session JSON, (b) Kocoro Desktop via the `schedule_run` SSE event, and (c) **the IM channel the schedule was created in** (Slack / Teams / Lark / Feishu / LINE), gated by the schedule's `broadcast` field. A schedule can never deliver to any other channel: the originating channel's routing context is snapshotted at creation time and is the only legitimate IM target.
- **Broadcast gate**: each schedule carries a `broadcast` setting with three values: `auto` (default), `on`, `off`. It decides WHETHER the reply is pushed back to the originating channel — never WHERE (the target is fixed at creation).
  - `auto`: smart default by where the schedule was created. IM-source schedules push their reply back to the channel they were created in; Desktop / TUI / CLI schedules stay local. Pre-2026-05-27 schedules without the field also stay local (safe default).
  - `on`: explicitly pin the push on for an IM-created schedule. Schedules created outside an IM chat (Desktop / TUI / CLI / raw HTTP) have no captured target and stay silent even with `on` — to get IM delivery, recreate the schedule from a conversation in the target channel.
  - `off`: never push. Use when the user creates a schedule from IM but explicitly wants it local-only.
  ...
  - **Proactive targeting**: a schedule created from an IM thread snapshots that thread's routing context (`im_status_context`) at creation time; when it pushes, Cloud delivers the reply back to the originating thread (Slack / Teams / Feishu / Lark / LINE). Transparent — no parameter. When the target cannot be honored (unsupported platform such as WeCom / Telegram, the channel was unbound/rebound since creation, or delivery fails), the message is dropped with a log — it is never widened to other channels. Legacy IM schedules created before 2026-06-03 (daemon v0.2.1) predate the snapshot and stay silent; recreate them from the target channel.
- **Thread anchoring (`thread`)**: when a schedule pushes to its IM channel, `thread` controls whether each run lands in one shared thread or as separate top-level messages. Three values: `auto` (default), `on`, `off`.
