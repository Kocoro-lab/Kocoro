...
struct TargetBoundInputRequestV1: Equatable, Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        switch action {
        case "type":
            guard let ref, strictMutationIdentity(ref), ref.count > 1,
                  ref.first == "e", ref.dropFirst().allSatisfy(\.isNumber),
                  let path, strictMutationIdentity(path),
                  path == "window[0]" || path.hasPrefix("window[0]/"),
                  let expectedRole, strictMutationIdentity(expectedRole),
                  let expectedFingerprint, strictMutationIdentity(expectedFingerprint),
...
func runTargetBoundInput(
    ...
    if let initialFailure = dependencies.authorityFailure(request) {
        guard targetBoundInputRestorableFocusFailuresV1.contains(initialFailure),
