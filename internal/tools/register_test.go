...
func TestRegisterAll_WithServerTools(t *testing.T) {
	...
	// Total: 37 local + 2 server = 39. doc-extract pdf/docx/xlsx/pptx are
...
func TestRegisterAll_ServerUnavailable(t *testing.T) {
	...
	schemas := reg.Schemas()
	if len(schemas) != 37 {
		t.Errorf("expected 37 local tools, got %d", len(schemas))
	}
}
...
func TestRegisterAll_LocalPriority(t *testing.T) {
	...
	// 37 local + 1 server (bash skipped) = 38. doc-extract pdf/docx/xlsx/pptx
