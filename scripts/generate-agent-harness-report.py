...
<section id="memory"><div class="eyebrow">02 / model-driven recall</div><h2>Memory：去掉 preflight 的收益主要是成本</h2><p class="sub">最终 2026-08-07 live 配对测试，3 repetitions、48 total runs。synthetic Ready service 只测路由、调用、答案忠实度、延迟与成本；不代表真实用户 bundle 的召回质量。</p><table><thead><tr><th>路径</th><th>总正确</th><th>正例路由</th><th>路由效率</th><th>正例答案</th><th>负例误召回</th><th>平均延迟</th><th>总成本</th></tr></thead><tbody>{memory_rows(memory_report)}</tbody></table>
