...
def main() -> None:
    ...
    # PROVENANCE WARNING: every value below except daemon_input_after and
    # daemon_total_after_ms (parsed from the supplied Koe runtime log) is a
    # hand-recorded constant. The tools-JSON char counts (62,765 -> 47,509),
    # tool counts (29 -> 22), bash schema token estimates (1,890 -> 462), and
    # the daemon "before" baselines have no raw artifact or test anchoring
    # this generator can re-derive them from. Treat the derived reduction
    # percentages as historical claims, not artifact-backed measurements.
    tool_surface = {
        "before_chars": 62_765,
        "before_tools": 29,
        "after_chars": 47_509,
